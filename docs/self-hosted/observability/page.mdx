...
- `ctld_rootfs_checkpoint_bytes` for input, output, and excluded byte distributions
...
Manager also records `apply_rootfs_checkpoint`, `activate_runtime`, and `resume_total` in `manager_sandbox_claim_phase_duration_seconds`. ctld emits a structured warning when a rootfs operation takes at least 10 seconds or handles a checkpoint of at least 512 MiB.
...
- `sandbox0_ctld_network_audit_ingest_events_total`
- `sandbox0_ctld_network_audit_ingest_batches_total`
