...
func TestSandboxDeletionWebhookOutboxEnqueueIntegration(t *testing.T) {
	...
	assert.Equal(t, deletionwebhook.DeliveryWindow, expiresAt.Sub(createdAt))
	...
	dispatcher := deletionwebhook.NewSandboxDeletionWebhookDispatcher(
		deletionwebhook.NewSandboxDeletionWebhookOutbox(pool),
		server.Client(),
		deletionwebhook.SandboxDeletionWebhookDispatcherConfig{WorkerID: "restarted-manager"},
		nil,
	)
