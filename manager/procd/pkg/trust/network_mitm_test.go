...
func TestConfigureNetworkMITMCATrustWritesBundleAndExportsTLSVars(t *testing.T) {
	dir := t.TempDir()
	...
	t.Setenv(NetworkMITMCAFileEnv, mitmPath)
	t.Setenv(NetworkMITMCABundleEnv, bundlePath)
	for _, name := range tlsBundleEnvVars {
		t.Setenv(name, "")
	}

	got, err := ConfigureNetworkMITMCATrust()
	if err != nil {
		t.Fatalf("ConfigureNetworkMITMCATrust: %v", err)
	}
	if got != bundlePath {
...
func TestConfigureNetworkMITMCATrustNoopsWithoutCAEnv(t *testing.T) {
	t.Setenv(NetworkMITMCAFileEnv, "")
