...
func (p *staticHeadersProvider) Resolve(
	...
	case egressauthstore.CredentialProjectionTypeHTTPHeaders:
		...
		response = newHTTPHeadersResolveResponse(req.AuthRef, headers, &expiresAt)
		return &resolveResult{
			Response: response,
			TTL:      ttl,
		}, nil
	case egressauthstore.CredentialProjectionTypePlaceholderSubstitution:
		...
		expiresAt := time.Now().UTC().Add(ttl)
		response = newPlaceholderSubstitutionResolveResponse(req.AuthRef, directive, &expiresAt)
		return &resolveResult{
			Response: response,
			TTL:      ttl,
		}, nil
...
func parseBindingTTL(cachePolicy *egressauthstore.CachePolicySpec, defaultTTL time.Duration) (time.Duration, bool) {
	if cachePolicy == nil {
...
func projectHTTPHeaders(projection *egressauthstore.HTTPHeadersProjection, values map[string]string) (map[string]string, error) {
	if projection == nil {
...
func projectPlaceholderSubstitution(projection *egressauthstore.PlaceholderSubstitutionProjection, values map[string]string) (*egressauth.PlaceholderSubstitutionDirective, error) {
	if projection == nil {
...
func (p *staticTLSClientCertificateProvider) Resolve(
	...
	if binding.Projection.Type != egressauthstore.CredentialProjectionTypeTLSClientCertificate || binding.Projection.TLSClientCertificate == nil {
	...
	expiresAt := time.Now().UTC().Add(ttl)
	return &resolveResult{
		Response: newTLSClientCertificateResolveResponse(req.AuthRef, &egressauth.TLSClientCertificateDirective{
...
func (p *staticUsernamePasswordProvider) Resolve(
	...
	if binding.Projection.Type != egressauthstore.CredentialProjectionTypeUsernamePassword || binding.Projection.UsernamePassword == nil {
	...
	expiresAt := time.Now().UTC().Add(ttl)
	return &resolveResult{
		Response: newUsernamePasswordResolveResponse(req.AuthRef, &egressauth.UsernamePasswordDirective{
...
func (p *staticSSHPrivateKeyProvider) Resolve(
	...
	if binding.Projection.Type != egressauthstore.CredentialProjectionTypeSSHProxy || binding.Projection.SSHProxy == nil {
	...
	expiresAt := time.Now().UTC().Add(ttl)
	return &resolveResult{
		Response: newSSHProxyResolveResponse(req.AuthRef, &egressauth.SSHProxyDirective{
