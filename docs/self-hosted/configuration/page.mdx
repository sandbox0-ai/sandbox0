...
When `spec.builtinTemplates` is omitted, infra-operator seeds two public builtin templates: `default` and `dins`. Set `spec.builtinTemplates` explicitly when you want to pin images, adjust pools, disable one of the builtins, or provide a full template `spec` for a runtime-specific Docker in Sandbox setup.
...
- `services.manager.config.rootfsPersistenceEnabled` creates a manager-owned SandboxVolume for each sandbox writable rootfs and writes the node runtime annotation contract; it requires `ctld` plus a compatible rootfs upperdir runtime or snapshotter on sandbox nodes, and clean restore prefers the previous node while that node-local upperdir owner is active
...
When `rootfsPersistenceEnabled` is enabled, `ctld` also serves a containerd proxy snapshotter on the host at `/run/containerd/sandbox0-rootfs-snapshotter.sock`. Configure containerd to register that proxy snapshotter and use it for sandbox workloads. The exact CRI plugin path differs across containerd versions, but the node-level intent is:

```toml
[proxy_plugins]
    [proxy_plugins.sandbox0-rootfs]
        type = "snapshot"
        address = "/run/containerd/sandbox0-rootfs-snapshotter.sock"

[plugins."io.containerd.cri.v1.runtime".containerd]
    snapshotter = "sandbox0-rootfs"
```
