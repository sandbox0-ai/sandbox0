...
func TestAggregatorFlushesAttributedOSSRequestWindows(t *testing.T) {
	...
	aggregator.ObserveRequestAttempt(objectstore.RequestAttempt{
		...
		Operation:  "GetObject",
		Key:        "sandbox-rootfs/team-1/sandbox-1/1/sha256/a.tar",
	...
	rootfsGet := windows["GetObject"]
	if rootfsGet.WindowType != meteringpkg.WindowTypeSandboxObjectStoreGetRequests ||
		rootfsGet.TeamID != "team-1" ||
		rootfsGet.SubjectType != meteringpkg.SubjectTypeRootFS ||
		rootfsGet.SubjectID != "sandbox-1" ||
		rootfsGet.SandboxID != "sandbox-1" ||
		rootfsGet.Value != 1 ||
		rootfsGet.Unit != meteringpkg.WindowUnitCount {
		t.Fatalf("unexpected rootfs GET window: %+v", rootfsGet)
	}
...
func TestAggregatorFlushFailureRetainsStableBatch(t *testing.T) {
	...
	aggregator.ObserveRequestAttempt(objectstore.RequestAttempt{
		...
		Key: "sandbox-rootfs/team-1/sandbox-1/1/sha256/a.tar", StatusCode: http.StatusOK,
	})
