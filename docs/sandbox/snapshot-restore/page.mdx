...
- `Claim with snapshot_id` adopts a normal template warm Pod when available, swaps only `procd` to the snapshot's digest-pinned metadata head, and verifies that head before process APIs are initialized.
...
Each new checkpoint excludes `/tmp` before its immutable diff and index are published. Snapshot heads from the previous replay architecture require an explicit migration and are not restored through a permanent compatibility path. See [Pause And Resume](/docs/sandbox/pause-resume#persistent-and-ephemeral-paths) for the complete path policy.
...
This is service-side encryption, not end-to-end encryption: manager, the active ctld, and the node-local rootfs snapshotter hold the installation key while serving sandbox operations. Encrypted range reads fetch only the header metadata and authenticated chunks covering the requested plaintext range; they do not download the entire rootfs object. Existing plaintext objects remain readable after encryption is enabled, while new object writes use the encrypted format; enabling encryption does not rewrite historical objects automatically. Self-hosted deployments can configure this behavior with `spec.storage.runtime.objectEncryptionEnabled`; see [Service and Runtime Config](/docs/sandbox/self-hosted/configuration#service-and-runtime-config).
