...
By default, manager uses `encrypted_pg`: credential source specs are encrypted before PostgreSQL persistence, and infra-operator mounts a stable per-installation encryption key into manager. During upgrade, legacy plaintext rows remain readable and are not rewritten automatically.

Platform operators can configure `spec.credentialVault` when credential material should live in a Vault-compatible backend. Infra-operator supports builtin OpenBao and external HashiCorp Vault:
...
API users keep sending the same write-only `spec` in both modes. Manager decides whether to store it in encrypted PostgreSQL or in the configured Vault backend.

To migrate legacy plaintext rows after an upgrade, run a one-shot manager command from the manager image with the same database and credential store configuration:

```bash
SERVICE=manager /usr/local/bin/entrypoint credential-store-backfill encrypted_pg
```

For a configured Vault backend, use:

```bash
SERVICE=manager /usr/local/bin/entrypoint credential-store-backfill hashicorp_vault
```

The command only rewrites rows that still have `storage_kind = plaintext_pg`; encrypted or Vault-backed rows are skipped.
