...
| `securityClass` | No | New templates use `privileged` by default; new `standard` templates are rejected. Existing `standard` sandboxes keep their original class across pause and resume. `privileged` grants guest-kernel capabilities inside gVisor but never bypasses runsc or exposes host devices. |
