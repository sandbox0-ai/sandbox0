...
`requireFUSERecovery: true` is the production-safe setting. It requires a kernel with Sandbox0's ANCK FUSE recovery patch set and refuses to publish portals if the kernel does not negotiate both recovery and request resend. The maintained kernel branch and immutable tags are published in [`sandbox0-ai/anck-kernel`](https://github.com/sandbox0-ai/anck-kernel). Install and boot that kernel on a drained canary node before enabling the setting. Drain a node before enabling nodefs, disabling it, or changing `nodeFSShardCount`; the shard count is immutable while that node has committed portals.
