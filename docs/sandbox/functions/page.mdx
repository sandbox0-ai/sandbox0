...
One common use case is keeping a coding-agent repository workspace warm. A GitHub push webhook calls a Sandbox Function, the handler refreshes the repository inside the sandbox rootfs and publishes a named rootfs snapshot. Later agent tasks claim or fork from that snapshot instead of cloning from scratch.
...
Sandbox0 injects the pinned `python-runner` artifact into the sandbox RootFS,
but the runner executes `python3` from the template image. Custom templates and
images therefore own the Python runtime used by Sandbox Functions. Older
Python versions are not supported; async handlers, streaming responses, and
...
Sandbox0 claims a fresh runtime allocation for the same sandbox identity,
restores the latest RootFS generation, and then executes the handler. The
sandbox ID and function `public_url` stay unchanged until the sandbox is
deleted or `hard_ttl` expires.
