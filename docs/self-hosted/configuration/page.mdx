...
`templateImageFS.enabled` imports every eligible template revision even when admission mode is `off`; only admitted revisions are selected for claims. `teamIds` applies only to private templates, while `templateIds` matches the logical template ID in either scope. Empty allowlists admit nothing. Set mode to `shared` and `sharedCarrierPool.enabled: true` after the cold cohort is healthy. On a dedicated green data plane, `rejectLegacyClaims: true` prevents an unmatched request from creating a legacy rootfs.
