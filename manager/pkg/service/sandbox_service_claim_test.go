...
func TestClaimIdlePodClearsStaleExpirationAnnotationsWhenTTLsDisabled(t *testing.T) {
	template := &v1alpha1.SandboxTemplate{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "template-a",
			Namespace: "ns-a",
	...
	readyPod := newClaimTestPod("ns-a", "idle-ready", "template-a", true)
	readyPod.Annotations[controller.AnnotationExpiresAt] = "2000-01-01T00:00:00Z"
	readyPod.Annotations[controller.AnnotationHardExpiresAt] = "2000-01-01T00:00:00Z"
	...
	if _, ok := pod.Annotations[controller.AnnotationExpiresAt]; ok {
		t.Fatalf("expires-at annotation retained: %#v", pod.Annotations)
	}
	if _, ok := pod.Annotations[controller.AnnotationHardExpiresAt]; ok {
		t.Fatalf("hard-expires-at annotation retained: %#v", pod.Annotations)
	}
}
...
func TestCreateNewPodUsesAbsoluteHardExpiresAtWithoutConfig(t *testing.T) {
	withClaimTestPublicKey(t)
	...
	if _, ok := pod.Annotations[controller.AnnotationExpiresAt]; ok {
		t.Fatalf("expires-at annotation is set with TTL disabled: %#v", pod.Annotations)
	}
}
...
func TestNormalizeSandboxConfigRejectsTTLGreaterThanHardTTL(t *testing.T) {
	ttl := int32(10)
	hardTTL := int32(5)
	err := normalizeSandboxConfigForPersistence(&sandboxstore.SandboxConfig{TTL: &ttl, HardTTL: &hardTTL})
	if err == nil {
		t.Fatal("normalizeSandboxConfigForPersistence() error = nil, want invalid lifecycle")
	}
	if !errors.Is(err, ErrInvalidClaimRequest) {
		t.Fatalf("normalizeSandboxConfigForPersistence() error = %v, want ErrInvalidClaimRequest", err)
	}
}
