...
ClickHouse is the canonical audit store. Audit facts are written to `sandbox_audit_events` with stable event IDs, trusted actor and producer identity, a canonical payload digest, and an Ed25519 gateway signature. PostgreSQL is not a second audit store. The infra-operator creates separate netd producer and audit-signing key pairs: netd receives only the producer private key, while cluster-gateway receives its public key and the independent signing key. Fsync-backed buffers are delivery state, not competing audit ledgers; records are deleted only after ClickHouse acknowledges the exact event. Cluster-gateway uses a persistent volume claim for pending result events, while each node-local netd instance keeps its flow buffer on that node.
...
Audit retention is measured from the cluster-gateway-owned `ingested_at` timestamp, not the producer's potentially delayed `occurred_at` timestamp. Backlogged events therefore receive the configured retention period after ClickHouse accepts them.
...
When the backend is enabled, infra-operator injects bounded internal ingest endpoints into ctld for sandbox-wide CRI runtime samples and manager for historical logs. When licensed audit is also enabled, it injects durable delivery-spool configuration into cluster-gateway and netd. Netd fsyncs an allow attempt before opening the upstream connection and waits for cluster-gateway to commit that attempt to ClickHouse. If the canonical store cannot acknowledge it, the connection is denied. Gateway mutation responses are held until their signed result is acknowledged; pending results return `503` and replay from disk.

Audit currently requires `spec.services.clusterGateway.replicas: 1`. Infra-operator mounts a `ReadWriteOnce` result-delivery PVC and uses the `Recreate` deployment strategy so the same buffer is detached and reattached before a replacement gateway starts. In production, set `audit.deliveryPersistence.storageClass` to durable, encrypted storage that supports cross-node volume reattachment; the cluster default is used when it is omitted. Netd flow buffers remain node-local and survive same-node Pod restarts, but not permanent node loss. All delivery records are removed only after ClickHouse acknowledgement. Preserve the generated audit-signing Secret for the full retention period; the built-in verifier currently uses that active public key, so destructive key replacement requires retaining the previous public key outside the cluster for historical offline verification.

Netd exports `sandbox0_netd_audit_ingest_events_total` and `sandbox0_netd_audit_ingest_batches_total`; alert on `persist_failed`, `corrupt`, `retrying`, `replay_failed`, and `ack_failed` results. Synchronous allow-attempt commits add one cluster-local ClickHouse round trip to each new audited flow. HTTP requests multiplexed over one existing connection remain protocol operations beneath that flow rather than independent network connections.
