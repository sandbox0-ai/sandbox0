...
After Function Gateway starts a restored `cmd` runtime context, it waits up to 30 seconds before sending user traffic. Function Gateway polls the owning `cluster-gateway` every 200 ms so readiness checks run inside the target cluster. On each poll, if the published service has `health_check.path`, the cluster gateway checks that path with `GET` and considers the runtime ready only after the endpoint returns HTTP `2xx`. If no health check is configured, the cluster gateway falls back to waiting for the service port to accept TCP connections.

Services published with `runtime.type: "warm_process"` reuse an existing running command context and skip startup readiness probing. Function Gateway still verifies that the named warm process exists and is running or paused before routing traffic.
