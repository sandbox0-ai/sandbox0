...
Only ready idle pods count toward pool capacity. This matters when your template uses sidecars to host a warm process.
...
| `readinessProbe` | Optional readiness probe for sidecars. Use it when a sidecar must finish warming up before the pod should count toward warm-pool capacity. |
| `startupProbe` | Optional startup probe for slow-starting sidecars. |

<Callout variant="info">
A pod that is `Running` but not `Ready` does not count as available warm-pool capacity and will not be hot-claimed.
</Callout>
...
Regular team-owned templates can still use public sidecar fields such as `command`, `args`, `env`, `resources`, `mounts`, `readinessProbe`, and `startupProbe`.
