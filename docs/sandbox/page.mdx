...
Sandbox0 exposes chart-ready runtime metrics alongside historical logs and network audit events. These APIs read asynchronous per-sandbox projections and never wake a paused sandbox. They are separate from platform service telemetry and the metering usage ledger. Centralized network audit collection and the events query/watch API require the enterprise `sandbox_audit` feature; logs, runtime metrics, and the metric catalog do not.
...
The events endpoint returns `403 feature_not_licensed` unless centralized audit is explicitly enabled and cluster-gateway has a valid enterprise license containing `sandbox_audit`.
...
Every returned event and log entry includes `team_id`, `sandbox_id`, `region_id`, `cluster_id`, `occurred_at`, `ingested_at`, and `cursor`. Event rows also include `source`, `event_type`, and `watermark`; log rows include `stream` and `message`. The response cursor is opaque and separate from each producer cursor.
...
ClickHouse-backed deployments store events, logs, and wide runtime samples in asynchronous per-sandbox projection tables with independent retention TTLs. When enterprise audit is enabled, netd publishes network audit events. Manager publishes logs, and node-local ctld instances collect sandbox-wide CRI runtime samples every 15 seconds with jitter regardless of the audit entitlement.
...
<Callout variant="info">
Metering windows are separate usage-ledger records for quota, showback, and billing export. Historical observability tables do not store metering windows, even when both use the same region ClickHouse component. File audit and SandboxVolume usage are not part of these endpoints. Writable rootfs metrics describe only the runtime container layer and are omitted when the CRI cannot report them.
</Callout>
