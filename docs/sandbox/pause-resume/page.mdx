...
The rootfs checkpoint covers files written inside the sandbox filesystem, including files outside mounted volumes. Template-declared Volume paths that are not bound in the claim remain rootfs-backed directories and are included in the checkpoint. Bound Sandbox Volume paths are excluded because their contents are owned by the Volume.

Rootfs checkpoints are stored as S0FS manifests and immutable segment objects in the rootfs object store. Each checkpoint is a complete writable-rootfs state, so Sandbox0 can restore the latest head directly without replaying legacy OCI layer diffs. It is different from a Sandbox Volume: it is tied to sandbox rootfs lifecycle and does not provide volume sharing or direct Volume file APIs. For named rootfs snapshots, restore, and fork operations, see [Snapshot And Restore](/docs/sandbox/snapshot-restore).
...
Resume restores the writable rootfs checkpoint onto a runtime pod created from the current template image. Sandbox0 requires the runtime, runtime handler, and snapshotter to match the checkpoint. Base image digest and containerd snapshot parent differences are treated as compatibility warnings so template base images can receive normal patch upgrades without making existing rootfs checkpoints unusable.
