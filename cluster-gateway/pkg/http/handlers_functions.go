...
func (s *Server) prepareAndDeployFunction(ctx context.Context, authCtx *gatewayauthn.AuthContext, functionIDOrSlug string, req functions.FunctionDeployRequest) (*functions.FunctionDeployResult, error) {
...
func (s *Server) compileFunctionRevision(ctx context.Context, authCtx *gatewayauthn.AuthContext, req functions.FunctionDeployRequest) (functions.FunctionRevisionSpec, functions.FunctionSource, error) {
	...
	case functions.RevisionSourceSandboxService:
		...
		return s.compileSandboxServiceRevision(ctx, authCtx, *req.Source.SandboxService, req.Slug)
	case "", functions.RevisionSourceSnapshot:
		...
		return specValue, functions.FunctionSource{Type: functions.RevisionSourceSnapshot, Snapshot: &functions.SnapshotRevisionSource{SnapshotIDs: snapshotIDs}}, nil
	default:
		return functions.FunctionRevisionSpec{}, functions.FunctionSource{}, fmt.Errorf("unsupported source type %q", req.Source.Type)
	}
}

func (s *Server) compileSandboxServiceRevision(ctx context.Context, authCtx *gatewayauthn.AuthContext, source functions.SandboxServiceSource, slug string) (functions.FunctionRevisionSpec, functions.FunctionSource, error) {
	...
	sandbox, err := s.managerClient.GetSandbox(ctx, sandboxID, authCtx.UserID, authCtx.TeamID)
	if err != nil {
		return functions.FunctionRevisionSpec{}, functions.FunctionSource{}, err
	}
	var selected *mgr.SandboxAppService
	...
	if blockers := mgr.SandboxAppServicePublishBlockers(*selected); len(blockers) > 0 {
	...
	for _, mount := range sandbox.Mounts {
		...
		mounts = append(mounts, functions.FunctionRevisionMount{SnapshotID: snapshot.Id, MountPath: mount.MountPoint, ReadOnly: true})
	}
	...
	return specValue, functions.FunctionSource{Type: functions.RevisionSourceSandboxService, SandboxService: &functions.SandboxServiceSource{SandboxID: sandboxID, ServiceID: serviceID}}, nil
}

func (s *Server) createFunctionSourceSnapshot(ctx context.Context, authCtx *gatewayauthn.AuthContext, volumeID, name string) (*apispec.Snapshot, error) {
	var snapshot apispec.Snapshot
	body := map[string]any{"name": name}
	path := "/sandboxvolumes/" + url.PathEscape(volumeID) + "/snapshots"
	if err := s.doFunctionJSON(ctx, s.cfg.StorageProxyURL, internalauth.ServiceStorageProxy, http.MethodPost, path, authCtx.TeamID, authCtx.UserID, append(authCtx.Permissions, gatewayauthn.PermSandboxVolumeWrite), body, &snapshot); err != nil {
...
func (s *Server) materializeFunctionVolume(ctx context.Context, teamID, userID string, snapshotID string) (string, error) {
	...
	body := map[string]any{"snapshot_id": snapshotID, "access_mode": "ROX"}
	if err := s.doFunctionJSON(ctx, s.cfg.StorageProxyURL, internalauth.ServiceStorageProxy, http.MethodPost, "/sandboxvolumes", teamID, userID, []string{gatewayauthn.PermSandboxVolumeCreate}, body, &volume); err != nil {
...
func writeDeployError(c *gin.Context, err error) {
	switch {
