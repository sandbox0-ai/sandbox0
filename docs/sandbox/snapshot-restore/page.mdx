...
Use [Volumes](/docs/sandbox/volume) when data must be mounted by multiple sandboxes, accessed directly through the Volume file API, or managed independently from sandbox rootfs lifecycle. Use sandbox rootfs snapshots when the default writable filesystem is the state you want to version.
...
Snapshots, snapshot claims, restore, and fork use the same persistence boundary as pause and resume. `/tmp` is ephemeral and is not captured or restored, while `/var/tmp` and ordinary writable rootfs paths remain persistent. A Sandbox Volume mounted beneath `/tmp` remains durable because Volume state is managed independently from rootfs checkpoints. Runtime-owned Kubernetes mounts such as `/procd`, `/procd-image`, webhook state, and other injected VolumeMounts are also excluded. UNIX domain socket paths are absent from the restored rootfs because live sockets belong to the discarded runtime process state. Named pipes (FIFOs) created inside gVisor are not guaranteed to persist because the runtime may not materialize their inode in the host overlay upper that rootfs capture observes.

The v3 runtime has no online legacy-layer reader. Existing old-format sandboxes and snapshots must be deleted or converted offline during the cutover. See [Pause And Resume](/docs/sandbox/pause-resume#persistent-and-ephemeral-paths) for the complete path policy.
