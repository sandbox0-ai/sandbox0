...
func TestRootFSMapsAbsoluteFileOperations(t *testing.T) {
	...
	if got := m.RootFS(); got != rootFS {
		t.Fatalf("RootFS() = %q, want %q", got, rootFS)
	}

	visibleDir := filepath.Join(t.TempDir(), "visible")
	visiblePath := filepath.Join(visibleDir, "nested", "file.txt")
	expectedPath := filepath.Join(rootFS, strings.TrimPrefix(filepath.Clean(visiblePath), string(filepath.Separator)))

	if err := m.WriteFile(visiblePath, []byte("rootfs data"), 0o644); err != nil {
	...
	if string(data) != "rootfs data" {
		t.Fatalf("rootfs file data = %q, want rootfs data", string(data))
	}
	if _, err := os.Stat(visiblePath); !os.IsNotExist(err) {
		t.Fatalf("visible host path stat err = %v, want not exist", err)
	}

	info, err := m.Stat(visiblePath)
	if err != nil {
		t.Fatalf("Stat() failed = %v", err)
	}
	if info.Path != visiblePath {
		t.Fatalf("Stat() path = %q, want %q", info.Path, visiblePath)
	}

	entries, err := m.ListDir(filepath.Dir(visiblePath))
	if err != nil {
		t.Fatalf("ListDir() failed = %v", err)
	}
	if len(entries) != 1 || entries[0].Path != visiblePath {
		t.Fatalf("ListDir() entries = %#v, want one entry at %q", entries, visiblePath)
	}

	movedVisiblePath := filepath.Join(visibleDir, "moved.txt")
	movedExpectedPath := filepath.Join(rootFS, strings.TrimPrefix(filepath.Clean(movedVisiblePath), string(filepath.Separator)))
	if err := m.Move(visiblePath, movedVisiblePath); err != nil {
		t.Fatalf("Move() failed = %v", err)
	}
	if _, err := os.Stat(expectedPath); !os.IsNotExist(err) {
		t.Fatalf("old rootfs path stat err = %v, want not exist", err)
	}
	if data, err := os.ReadFile(movedExpectedPath); err != nil || string(data) != "rootfs data" {
		t.Fatalf("moved rootfs data = %q err=%v, want rootfs data", string(data), err)
	}

	if err := m.Remove(movedVisiblePath); err != nil {
		t.Fatalf("Remove() failed = %v", err)
	}
	if _, err := os.Stat(movedExpectedPath); !os.IsNotExist(err) {
		t.Fatalf("removed rootfs path stat err = %v, want not exist", err)
	}
}

func TestRootFSKeepsRelativeTraversalInsideRootFS(t *testing.T) {
	rootPath := filepath.Join(t.TempDir(), "workspace")
	rootFS := filepath.Join(t.TempDir(), "rootfs")
	...
	virtualPath := filepath.Clean(filepath.Join(rootPath, relativePath))
	expectedPath := filepath.Join(rootFS, strings.TrimPrefix(virtualPath, string(filepath.Separator)))

	if err := m.WriteFile(relativePath, []byte("inside rootfs"), 0o644); err != nil {
		t.Fatalf("WriteFile() failed = %v", err)
	}
	if data, err := os.ReadFile(expectedPath); err != nil || string(data) != "inside rootfs" {
		t.Fatalf("rootfs data = %q err=%v, want inside rootfs", string(data), err)
	}
	if _, err := os.Stat(virtualPath); !os.IsNotExist(err) {
		t.Fatalf("virtual host path stat err = %v, want not exist", err)
	}
}

func TestRootFSWatchEventsUseSandboxVisiblePaths(t *testing.T) {
	rootPath := filepath.Join(t.TempDir(), "workspace")
	rootFS := filepath.Join(t.TempDir(), "rootfs")
	...
	visibleDir := filepath.Join(t.TempDir(), "watch")
	visiblePath := filepath.Join(visibleDir, "file.txt")
	events := make(chan WatchEvent, 1)

	watcher, unsubscribe, err := m.SubscribeWatch(visibleDir, true, func(event WatchEvent) {
		events <- event
	})
	if err != nil {
		t.Fatalf("SubscribeWatch() failed = %v", err)
	}
	defer unsubscribe()
	...
	select {
	case event := <-events:
		if event.WatchID != watcher.ID {
			t.Fatalf("WatchID = %q, want %q", event.WatchID, watcher.ID)
		}
		if event.Path != visiblePath {
			t.Fatalf("event path = %q, want %q", event.Path, visiblePath)
		}
