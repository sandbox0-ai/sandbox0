...
After authentication, the TCP connection and every active shell, exec, or file-transfer channel consume the team's region-wide `active_connection_count` quota. Opening a new channel at the exact limit is rejected. If the backing quota lease is lost or cannot renew, the gateway closes the affected channel or connection instead of letting it continue uncounted.
...
- SSH channel payload is included in the team's region-wide network byte quota. Client-to-sandbox data consumes `network_ingress_bytes`; sandbox-to-client data consumes `network_egress_bytes`.
- Payload is counted once at the SSH channel boundary for shell, exec, SFTP, and SFTP-backed SCP. SSH handshake/control frames and the internal gateway-to-`procd` WebSocket are not charged again.
