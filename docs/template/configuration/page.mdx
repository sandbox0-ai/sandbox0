...
| `egress.protocolRules` | array | — | Protocol-aware controls for allowed traffic, such as MCP tool allowlists. |
...
See [Network](/docs/sandbox/network) and [Protocol Controls](/docs/sandbox/protocol-controls) for traffic behavior, and [Credential](/docs/credential) for bindings and egress auth.
...
| `mainContainer.devices` | Sandbox device resources requested from the node-local `ctld` device plugin. Supported values are `fuse` and `net-tun`. |
...
<Callout variant="warning">
Device passthrough depends on the configured Kubernetes runtime. These devices do not grant the namespace, seccomp, AppArmor, or privilege settings that container engines may also require. Validate container-engine templates on the target runtimeClass before exposing them to users.
</Callout>
