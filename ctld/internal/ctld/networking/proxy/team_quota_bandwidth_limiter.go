...
const defaultNetworkingBandwidthRedisKeyPrefix = "sandbox0:quota:network"
...
func newQuotaTeamBandwidthLimiter(ctx context.Context, cfg *config.NetworkRuntimeConfig, policies quota.PolicyStore) (*quotaTeamBandwidthLimiter, error) {
	...
	if prefix == "" {
		prefix = defaultNetworkingBandwidthRedisKeyPrefix
	}
...
func WithTeamQuotaBandwidth(ctx context.Context, cfg *config.NetworkRuntimeConfig, policies quota.PolicyStore) (ServerOption, error) {
