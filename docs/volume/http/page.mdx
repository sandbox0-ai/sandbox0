...
Import a `tar.gz` or raw `tar` archive into a Volume. This is the fastest direct API path for build outputs with many files because it avoids one HTTP request per file.

<Endpoint method="POST">
/api/v1/sandboxvolumes/{'{id}'}/files/archive?path=/workspace&format=tar.gz&overwrite=false
</Endpoint>
...
| Field | Type | Description |
|-------|------|-------------|
| `path` | string | Destination directory. Defaults to `/` |
| `format` | string | `tar.gz` or `tar`. Defaults to `tar.gz` |
| `overwrite` | boolean | Replace existing paths when true. Defaults to false |

The archive may contain directories, regular files, and symlinks. Absolute paths and `..` path traversal entries are rejected.
  ...
  tabs={[
    {
      label: "Go",
      language: "go",
      code: `file, err := os.Open("bundle.tar.gz")
if err != nil {
    log.Fatal(err)
}
defer file.Close()

result, err := client.UploadVolumeArchive(ctx, volumeID, file, &sandbox0.VolumeArchiveUploadOptions{
    Path:      "/workspace",
...
fmt.Printf("imported %d files\\n", result.Files)`
    },
    {
      label: "CLI",
      language: "bash",
      code: `s0 volume files upload-archive vol_abc123 bundle.tar.gz /workspace \\
    --format tar.gz \\
    --overwrite`
    }
  ]}
