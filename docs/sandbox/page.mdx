...
Every audit fact includes a stable `event_id`, schema version, trusted actor, action, resource, producer, operation correlation, outcome, and bounded request metadata. `integrity` contains the canonical SHA-256 payload digest, Ed25519 signature, signing key ID, and inline verification status. List and watch responses recompute each returned row's digest and verify its signature. An exact `event_id` lookup fetches enough rows to report `conflict` when the same stable ID appears with multiple payload hashes.
