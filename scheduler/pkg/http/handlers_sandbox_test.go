...
func TestSelectClusterForTemplateSkipsIdleClusterWithoutClaimStartCapacity(t *testing.T) {
	tpl := newRoutingTemplate("tmpl-a")
	clusterTemplateID := naming.TemplateNameForCluster(tpl.Scope, tpl.TeamID, tpl.TemplateID)
	server := newRoutingTestServer(
		tpl,
		...
		[]*template.Cluster{
			newRoutingCluster("cluster-a", 10),
			newRoutingCluster("cluster-b", 1),
		},
		&fakeRoutingReconciler{
			templateIdle: map[string]map[string]int32{
				"cluster-a": {clusterTemplateID: 3},
				"cluster-b": {clusterTemplateID: 1},
			},
			templateStatsAge: map[string]time.Duration{
				"cluster-a": time.Second,
			...
			clusterSummaries: map[string]*templreconciler.ClusterSummary{
				"cluster-a": {ClaimStartLimiterBackend: "postgres_advisory_lock", ClaimStartLimit: 80, ClaimStartInFlight: 80, ClaimStartAvailable: 0},
				"cluster-b": {ClaimStartLimiterBackend: "postgres_advisory_lock", ClaimStartLimit: 80, ClaimStartInFlight: 75, ClaimStartAvailable: 5},
			},
			clusterSummaryAge: map[string]time.Duration{
				"cluster-a": time.Second,
				"cluster-b": time.Second,
...
func TestSelectClusterForTemplateUsesClaimStartCapacityBeforeHeadroom(t *testing.T) {
	tpl := newRoutingTemplate("tmpl-a")
	server := newRoutingTestServer(
		tpl,
		[]*template.TemplateAllocation{
		...
		&fakeRoutingReconciler{
			clusterSummaries: map[string]*templreconciler.ClusterSummary{
				"cluster-a": {SandboxNodeCount: 10, TotalNodeCount: 10, TotalPodCount: 1, ClaimStartLimiterBackend: "postgres_advisory_lock", ClaimStartLimit: 80, ClaimStartInFlight: 79, ClaimStartAvailable: 1},
				"cluster-b": {SandboxNodeCount: 1, TotalNodeCount: 1, TotalPodCount: 9, ClaimStartLimiterBackend: "postgres_advisory_lock", ClaimStartLimit: 30, ClaimStartInFlight: 20, ClaimStartAvailable: 10},
			},
			clusterSummaryAge: map[string]time.Duration{
				"cluster-a": time.Second,
				"cluster-b": time.Second,
	...
	if selectedBy != "claim_start_capacity" {
		t.Fatalf("selectedBy = %q, want %q", selectedBy, "claim_start_capacity")
	}
}
