...
Sandbox logs, metrics, lifecycle events, and audit events are exposed through one per-sandbox observability API. The query API reads an asynchronous per-sandbox projection that can be backed by ClickHouse or another query store. When no backend is configured, these endpoints return `503 unavailable`. When the backend is configured but cannot be queried, the same endpoints return `503 unavailable` with a backend-unavailable error.
...
| `context_id` | string | Log and metric context filter |
| `stream` | string | Log stream filter: `stdout`, `stderr`, or `pty` |
| `name` | string | Metric name filter; may be repeated |
| `names` | string | Comma-separated metric name filter |
| `source` | string | Event producer filter: `manager`, `netd`, or `procd` |
| `event_type` | string | Event projection filter: `lifecycle`, `network_audit`, or `runtime_stats` |
| `outcome` | string | Event outcome filter: `completed`, `denied`, `error`, `succeeded`, or `failed` |

Every returned event, log entry, and metric sample includes `team_id`, `sandbox_id`, `region_id`, `cluster_id`, `occurred_at`, `ingested_at`, and `cursor`. Event rows also include `source`, `event_type`, and `watermark`; log rows include `stream` and `message`; metric rows include `name` and `value`. The response cursor is opaque and separate from each producer cursor.

Set `watch=true` to stream matching records as NDJSON in ingestion order. Without `cursor` or `start_time`, a watch stream starts at request time and does not replay historical rows. When `watch=true`, `end_time` is not supported. A watch stream emits record lines (`event`, `log`, or `metric_sample`), `watermark` lines with a resume cursor, periodic `heartbeat` lines, and an `error` line before termination if the backend fails after the stream starts.

ClickHouse-backed deployments store events, logs, and metric samples in asynchronous per-sandbox projection tables with independent retention TTLs. Netd network audit events arrive through an asynchronous internal ingest sink when enabled by the deployment. Runtime stats samples are published by the sandbox runtime metric producer.

Query endpoints return `503` when the backend is disabled or unavailable.

<Callout variant="info">
Metering windows are separate usage-ledger records for quota, showback, and billing export. Historical observability tables do not store metering windows, even when the deployment also uses the same region ClickHouse component for metering. The current operator rejects ClickHouse metering backend selection until all metering write paths have been migrated. File audit is not part of these endpoints.
</Callout>
