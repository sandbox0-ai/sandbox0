...
func TestEngineReplaceStateFromOlderManifestAdvancesLatestHead(t *testing.T) {
	ctx := context.Background()
	store := newPrefixedRecordingStore(t, "vol-restore-old")
	volumeID := "vol-restore-old"
	...
	if _, err := engine.Write(node.Inode, 0, []byte("v1")); err != nil {
		t.Fatalf("Write(v1) error = %v", err)
	}
	first, err := engine.EnsureMaterialized(ctx)
	if err != nil {
		t.Fatalf("EnsureMaterialized(first) error = %v", err)
	}
	firstState := cloneState(first.State)

	if _, err := engine.Write(node.Inode, 0, []byte("v2")); err != nil {
		t.Fatalf("Write(v2) error = %v", err)
	}
	second, err := engine.EnsureMaterialized(ctx)
	if err != nil {
		t.Fatalf("EnsureMaterialized(second) error = %v", err)
	}
	if second.ManifestSeq <= first.ManifestSeq {
		t.Fatalf("second manifest seq = %d, want > %d", second.ManifestSeq, first.ManifestSeq)
	}

	if err := engine.ReplaceState(firstState); err != nil {
		t.Fatalf("ReplaceState(first) error = %v", err)
	}
	restored, err := engine.EnsureMaterialized(ctx)
	if err != nil {
		t.Fatalf("EnsureMaterialized(restored) error = %v", err)
	}
	if restored.ManifestSeq <= second.ManifestSeq {
		t.Fatalf("restored manifest seq = %d, want > %d", restored.ManifestSeq, second.ManifestSeq)
	}

	_, latestManifest, err := NewMaterializer(volumeID, store, nil).LoadLatestState(ctx)
	if err != nil {
		t.Fatalf("LoadLatestState() error = %v", err)
	}
	if latestManifest.ManifestSeq != restored.ManifestSeq {
		t.Fatalf("latest manifest seq = %d, want restored seq %d", latestManifest.ManifestSeq, restored.ManifestSeq)
	}

	reopened, err := Open(ctx, Config{
		VolumeID:    volumeID,
		WALPath:     filepath.Join(t.TempDir(), "reopened.wal"),
		ObjectStore: store,
	})
	if err != nil {
		t.Fatalf("Open(reopened) error = %v", err)
	...
	defer reopened.Close()
	reopenedNode, err := reopened.Lookup(RootInode, "state.txt")
	if err != nil {
		t.Fatalf("Lookup(reopened) error = %v", err)
	}
	payload, err := reopened.Read(reopenedNode.Inode, 0, reopenedNode.Size)
	if err != nil {
		t.Fatalf("Read(reopened) error = %v", err)
	}
	if string(payload) != "v1" {
		t.Fatalf("Read(reopened) = %q, want v1", payload)
	}
}
