...
func TestRequestObservingHTTPClientRecordsEachProviderAttempt(t *testing.T) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, "https://oss.example/runtime-bucket/sandbox-rootfs/team-1/sandbox-1/1/sha256/a.tar", nil)
	if err != nil {
	...
	for _, attempt := range attempts {
		...
		if attempt.Key != "sandbox-rootfs/team-1/sandbox-1/1/sha256/a.tar" {
...
func TestOSSStoreEmitsSDKRequestAttempt(t *testing.T) {
	...
	if err := store.Put("sandbox-rootfs/team-1/sandbox-1/object", strings.NewReader("data")); err != nil {
		t.Fatalf("Put() error = %v", err)
	}

	if gotPath != "/runtime-bucket/sandbox-rootfs/team-1/sandbox-1/object" {
		t.Fatalf("request path = %q", gotPath)
	}
	if got.Provider != TypeOSS || got.Bucket != "runtime-bucket" ||
		got.Operation != "PutObject" || got.Key != "sandbox-rootfs/team-1/sandbox-1/object" ||
...
func TestRequestObjectKeyPreservesRepeatedBucketSegment(t *testing.T) {
	req, err := http.NewRequest(
		http.MethodGet,
		"https://oss.example/runtime-bucket/sandbox-rootfs/team-1/sandbox-1/runtime-bucket/object",
		nil,
	)
	if err != nil {
	...
	want := "sandbox-rootfs/team-1/sandbox-1/runtime-bucket/object"
	if got != want {
...
func TestRequestObservingHTTPClientPreservesTransportError(t *testing.T) {
	...
	req, err := http.NewRequestWithContext(
		ctx,
		http.MethodPut,
		"https://oss.example/runtime-bucket/sandbox-rootfs/team-1/sandbox-1/1/head",
