...
const (
	defaultCtldApplyTimeout = 5 * time.Second
)

// CtldProviderConfig configures policy-apply waits for the ctld network runtime.
type CtldProviderConfig struct {
...
// CtldProvider waits for the ctld network runtime to apply policies.
type CtldProvider struct {
...
// NewCtldProvider creates a Provider backed by the ctld network runtime.
func NewCtldProvider(
	podInformer coreinformers.PodInformer,
	podLister corelisters.PodLister,
	config CtldProviderConfig,
	logger *zap.Logger,
) Provider {
	if logger == nil {
		logger = zap.NewNop()
	...
	if applyTimeout == 0 {
		applyTimeout = defaultCtldApplyTimeout
	}
...
func (p *CtldProvider) Name() string { return "ctld" }

func (p *CtldProvider) EnsureBaseline(ctx context.Context, namespace string) error {
	return nil
}

func (p *CtldProvider) ApplySandboxPolicy(ctx context.Context, input SandboxPolicyInput) error {
	if p.podLister == nil {
...
func (p *CtldProvider) RemoveSandboxPolicy(ctx context.Context, namespace, sandboxID string) error {
	return nil
}

func (p *CtldProvider) waitForAppliedHashes(
	ctx context.Context,
	namespace string,
	podName string,
	expectedNetworkHash string,
...
func (p *CtldProvider) appliedHashesMatch(
	pod *corev1.Pod,
	expectedNetworkHash string,
) bool {
...
func (p *CtldProvider) registerPodHandlers() {
...
func (p *CtldProvider) handlePodEvent(obj any) {
...
func (p *CtldProvider) addWaiter(key string, waiter *policyWaiter) {
	p.waitersMu.Lock()
...
func (p *CtldProvider) removeWaiter(key string, waiter *policyWaiter) {
	p.waitersMu.Lock()
	defer p.waitersMu.Unlock()
	waiters := p.waiters[key]
	if waiters == nil {
...
func (p *CtldProvider) completeWaiter(key string, waiter *policyWaiter) {
	p.waitersMu.Lock()
	defer p.waitersMu.Unlock()
	p.completeWaiterLocked(key, waiter)
}

func (p *CtldProvider) completeWaiterLocked(key string, waiter *policyWaiter) {
	waiter.once.Do(func() {
		close(waiter.done)
	})
	waiters := p.waiters[key]
...
func (p *CtldProvider) tick(ticker *time.Ticker) <-chan time.Time {
	if ticker == nil {
		return nil
	}
	return ticker.C
...
func (p *CtldProvider) networkPolicyHash(spec *v1alpha1.NetworkPolicySpec) (string, error) {
