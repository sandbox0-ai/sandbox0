...
Use the streaming helpers when you want command output as it is produced instead of waiting for a final `output_raw` response.
...
for await (const output of stream.outputs()) {
    ...
    },
    {
      label: "CLI",
      language: "bash",
      code: `s0 sandbox exec sb_abc123 --stream -- sh -c 'echo hello && echo warn >&2'`
    }
  ]}
/>
...
| `done` | Completion marker for a request identified by `request_id` | `{"type": "done", "request_id": "req-1"}` |
