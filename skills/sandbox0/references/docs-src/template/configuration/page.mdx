...
Only ready idle pods count toward pool capacity. This matters when your template declares warm processes, because procd readiness stays unavailable until those processes are running.
...
<Callout variant="info">
Warm processes are intended to stay running. If a configured warm process exits, procd readiness fails and the pod is not hot-claimed.
</Callout>
