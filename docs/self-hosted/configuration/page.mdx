...
Public and automatic soft-TTL pause are routed to the selected Nomad backend. Before asking Nomad to stop the exact allocation, manager commits a deterministic, non-cancelable planned-retirement lifecycle bound to the sandbox, claim, slot, writer grant, and current generation. The stop removes server scheduling ownership without forcing client garbage collection, so the task driver can seal and publish the dirty RootFS generation. Background reconciliation waits for that publication to atomically commit the paused sandbox and retired writer before it makes the slot immediately eligible for terminal reconciliation. The terminal reconciler preserves the exact regional retirement operation, kind, and proof instead of replacing a successful planned publication with crash abandonment. The root-owned ctld Nomad runtime either finishes the matching durable planned session or, if publication already reclaimed and forgot that session, rejects every conflicting local writer and uses the immutable slot journal to prove runsc, stable-mount, RootFS-writer, and network-policy absence. It journals the complete cleanup proof before returning it, then direct Nomad client garbage collection and slot finalization can retry independently of the task-driver plugin. Resume resolves the stored runtime profile and network policy before mutation, reserves active-sandbox quota in the same PostgreSQL transaction as a deterministic paused-head lifecycle, claims a fresh compatible slot with the next writer epoch, and makes the sandbox active only after the exact slot has committed authenticated procd command readiness. Exact lifecycle and already-active retries do not depend on reloading quota policy and cannot select a second slot.
...
Nomad nodes must run the ctld A/B pair as root host systemd services, not as Kubernetes Pods. The pair shares `/var/lib/sandbox0/ctld`; only the elected primary opens Bolt, NBD, mount, network, and Unix-socket resources. Ctld and the task driver must observe the same host mount namespace, `/opt/nomad`, and the canonical `/run/netns` directory; `/var/run/netns` is rejected on hosts where `/var/run` is a symlink. Use the versioned assets under `deploy/nomad/ctld`; the installer provisions kernel modules, NBD bounds, sysctls, the Nomad dependency, and B-then-A role-aware rollout. Systemd filesystem/device isolation directives that create a private mount namespace are intentionally forbidden.
