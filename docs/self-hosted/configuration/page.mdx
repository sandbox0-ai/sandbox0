...
The manager CRD includes an experimental `nodeAuthority` listener for the in-progress Nomad + gVisor runtime. It is disabled by default and is not a production-ready alternative to the Kubernetes data plane yet. Enabling the listener does not enable Nomad sandbox claims by itself.
...
Optional terminal reconciliation additionally requires `terminal.controlSecretName`. That Secret must contain `nomad-endpoints.json` and every CA, client certificate, client key, and ACL-token file referenced by the catalog, using absolute paths below `/etc/sandbox0/node-authority/control`. Every manager replica runs the idempotent terminal scan so the replica holding a node's outbound stream can make progress; PostgreSQL fences and deterministic operation IDs remain authoritative.

Do not enable this route without separately provisioning short-lived node credentials, strict Nomad ACLs, and the node agent. The current static certificate identity catalog is transitional, and the complete regional claim route, cross-replica claim dispatch, privileged multi-node validation, and long-running load gates are still required before production cutover.
