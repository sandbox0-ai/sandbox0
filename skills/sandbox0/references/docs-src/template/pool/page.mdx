...
1. The manager first tries to claim a Kubernetes-ready idle pod (`claimIdlePod`)
...
In practice, “available” means `Pod Ready`, not merely `Pod Running`.
...
Warm-pool capacity is counted from idle pods that reach Kubernetes readiness.
...
- Sandbox0 now owns the readiness gate for sandbox pods, so sidecar `readinessProbe` is not part of the public template surface

If a template uses a sidecar to host a warm process, keep startup checks on the sidecar and let Sandbox0 drive pod readiness:
...
In this model, the sidecar can still express slow-start behavior with `startupProbe`, but the pod only counts as warm-pool capacity after Sandbox0 marks its readiness gate ready.
