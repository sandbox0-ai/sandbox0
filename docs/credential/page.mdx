...
For Vault-backed storage, configure `spec.credentialVault` on `Sandbox0Infra` and create sources with `storageKind: hashicorp_vault`. Builtin mode installs OpenBao; external mode connects to a platform-provided HashiCorp Vault endpoint.
