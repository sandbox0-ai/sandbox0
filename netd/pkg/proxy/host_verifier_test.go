...
func TestVerifyClassifiedHostKeepsKnownTrafficWhenVerified(t *testing.T) {
	...
	classification = verifyClassifiedHost(stubHostVerifier{ok: true}, classification)
	if classification.UnknownReason != "" {
		t.Fatalf("expected known classification, got unknown reason %q", classification.UnknownReason)
	}
}

func TestVerifyClassifiedHostMarksMismatchAsUnknown(t *testing.T) {
	classification := classifyKnownTraffic("tcp", "http", net.ParseIP("8.8.8.8"), 443, "example.com")
	classification = verifyClassifiedHost(stubHostVerifier{}, classification)
	if classification.UnknownReason != "host_dest_mismatch" {
		t.Fatalf("unexpected unknown reason %q", classification.UnknownReason)
	}
}

func TestVerifyClassifiedHostMarksLookupFailureAsUnknown(t *testing.T) {
	classification := classifyKnownTraffic("tcp", "http", net.ParseIP("8.8.8.8"), 443, "example.com")
	classification = verifyClassifiedHost(stubHostVerifier{err: fmt.Errorf("lookup failed")}, classification)
	if classification.UnknownReason != "host_resolution_failed" {
		t.Fatalf("unexpected unknown reason %q", classification.UnknownReason)
	}
}
...
func TestVerifyClassifiedHostMismatchFailsClosedUnderBlockAll(t *testing.T) {
	...
	if decision.ClassifierResult != "unknown" {
		t.Fatalf("expected unknown classifier result, got %s", decision.ClassifierResult)
	}
...
func TestVerifyClassifiedHostMismatchFallsBackUnderAllowAll(t *testing.T) {
	compiled := &policy.CompiledPolicy{
