...
| Environment variable credentials | Stored as sandbox environment placeholders and resolved by Sandbox0 egress auth. They are available to sandbox-backed runtimes, not as SDK client or resident harness configuration secrets. External sandbox-as-tool harnesses reject them today. |
...
| Pi harness | Requires an Anthropic Messages-compatible endpoint and uses Sandbox0 as a tool target. |
...
`claude` and `codex` use agent in sandbox behavior. `openai-agents` and `pi` use sandbox as tool behavior, so the resident runtime can start a run before a Sandbox0 sandbox is claimed.
