...
The claim Secret must contain `runtime-profiles.json` and an exact 32-byte binary `writer-token.key`. The profile catalog strictly maps a public CPU/memory shape to one cluster and one complete runtime compatibility digest; unknown fields, ambiguous resource shapes, mutable image references, and unavailable block-COW base artifacts fail closed. Keep the writer key stable across retries and rollouts.
