...
Use this only after draining and upgrading a canary node with the maintained [`sandbox0-ai/anck-kernel`](https://github.com/sandbox0-ai/anck-kernel) recovery build. ctld fails closed if recovery and resend are not negotiated. See [Self-hosted Configuration](/docs/self-hosted/configuration) for the lifecycle and shard-count constraints.
