...
- `manager_sandbox_claim_phase_duration_seconds` with `prepare_rootfs_head`, `claim_idle_pod`, `create_new_pod`, `wait_for_rootfs_head`, `activate_runtime`, and `resume_total` phases

Resume no longer has a ctld apply/replay phase or a node-local full-object cache. Check the rootfs snapshotter DaemonSet readiness, containerd's `sandbox0` plugin registration, and structured object-range read errors when `wait_for_rootfs_head` is slow or fails. Rootfs object requests use the same PostgreSQL metering outbox as ctld requests. Ctld emits a structured warning when a checkpoint save takes at least 10 seconds or handles at least 512 MiB.
