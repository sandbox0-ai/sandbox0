...
| `credentialBindings` | array | Runtime credential bindings for egress auth (optional) |
...
sandbox.update_network_policy(SandboxNetworkPolicy(mode=SandboxNetworkPolicyMode.ALLOW_ALL))
...
sandbox.update_network_policy(
    SandboxNetworkPolicy(
        mode=SandboxNetworkPolicyMode.ALLOW_ALL,
        egress=NetworkEgressPolicy(
