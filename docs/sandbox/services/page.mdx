...
Runtime hooks are optional blocking callbacks executed by Function Gateway after a runtime sandbox is claimed and the service runtime context is available. The first supported phase is `post_claim`; it sends a JSON metadata payload to an HTTP path on the service through the internal function runtime proxy. Use it for generic runtime initialization that depends on mounted Volumes or revision metadata.

Set `runtime.skip_readiness_check` when the hook is the readiness boundary and the runtime should serve without an extra HTTP or TCP readiness probe. This is intended for long-lived warm processes that already exist in the template pod and perform initialization through a post-claim hook.
