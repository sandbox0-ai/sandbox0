...
Use `readinessProbe` when a sidecar must finish warming up before the pod should count as ready warm capacity. Use `startupProbe` for slow-starting sidecars that need extra time before regular readiness checks should matter.
