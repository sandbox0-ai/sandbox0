...
Self-hosted deployments must run `ctld`, the external rootfs snapshotter, and the `gvisor-rootfs` containerd handler on sandbox nodes. Pause publishes the writable rootfs Head and releases the runtime pod; it does not preserve process state.
</Callout>
...
Sandbox writes stay on a native node-local overlayfs upper. `ctld` continuously converts changed upper entries into team-scoped immutable chunks and metadata, while each published Head describes the complete logical writable rootfs. After manager atomically commits the filesystem-to-Head pointer, resume attaches that Head lazily through the external snapshotter and creates a fresh native overlay upper. Resume does not replay a historical layer chain.
...
| The node rootfs snapshotter process changes | The node becomes unavailable for new claims until the replacement passes containerd, object-store, and FUSE readiness. Manager then durably seals and recreates affected runtimes against the replacement instance; existing overlays are not transparently reconnected |
...
- Runtime-owned mounts are excluded from rootfs capture. This includes the `/procd` and `/procd-image` mounts, webhook state, and any other Kubernetes VolumeMount injected into the sandbox Pod.
- UNIX domain sockets are process state, not restorable filesystem data. A socket created in the writable rootfs is absent after resume, including when an older lower layer had an entry at the same path.
- Named pipes (FIFOs) created inside the current gVisor runtime are not guaranteed to persist. gVisor can keep the FIFO inode in its virtual filesystem without materializing it in the host overlay upper that `ctld` captures. Use a regular file or Sandbox Volume for durable state instead of relying on a FIFO path surviving runtime replacement.
- File reads alone do not make access-time changes durable across a Head publication. Applications that need a persistent last-access signal should write explicit metadata.

This policy is applied by continuous capture and by every newly published complete Head. The v3 reader does not accept legacy checkpoint formats; upgrade or cutover procedures must delete old sandboxes or migrate them offline before enabling the new runtime.
