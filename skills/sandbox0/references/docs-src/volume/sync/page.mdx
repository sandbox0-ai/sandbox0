...
3. define a Sandbox template mount path
4. claim a Sandbox with that Volume bound to the declared path
5. work locally, in the sandbox, or both
...
Once the Volume is attached locally, claim a Sandbox from a template that declares the workspace mount path:

```bash
s0 sandbox claim --template workspace --mount vol_abc123xyz:/workspace
```
...
3. claim a sandbox with that Volume bound to a template-declared mount path whenever you need remote execution or agent help
...
2. claim a sandbox with the same Volume bound at `/workspace`
...
<CardGrid>
  <LinkCard
    title="Workspace Sync for AI Agents"
    href="/blog/2026-03/workspace-sync-for-ai-agents"
    cta="Read the Blog"
  >
