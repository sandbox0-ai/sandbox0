...
func TestClaimLayoutPolicyIsExactAndKeepsGenericCarrierIdentity(t *testing.T) {
	for _, configured := range []string{"", rootfsimporter.XFSFileRangesV1} {
		for _, available := range []string{"", rootfsimporter.XFSFileRangesV1} {
			t.Run(configured+"-"+available, func(t *testing.T) {
				f := newClaimServiceFixture(t)
				f.config.RootFSFormatGeneration = 2
				f.config.RootFSImportDataLayoutPolicy = configured
				f.store.artifact.FormatGeneration = 2
				f.store.artifact.ImportDataLayoutPolicy = available
				var err error
				...
				require.Equal(t, configured, f.store.artifactRequirements[0].ImportDataLayoutPolicy)
...
func TestClaimLayoutPolicyDoesNotChangeCommittedSourceRequirements(t *testing.T) {
	f := newClaimServiceFixture(t)
	f.config.RootFSFormatGeneration = 2
	f.config.RootFSImportDataLayoutPolicy = rootfsimporter.XFSFileRangesV1
	var err error
	f.service, err = New(f.config)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Empty(t, requirements.ImportDataLayoutPolicy)
	require.Empty(t, requirements.SourceOCIRef)
	require.Zero(t, requirements.ImportDataRangeBytes)
	f.config.RootFSImportDataLayoutPolicy = "unknown"
	_, err = New(f.config)
	require.Error(t, err)
}
