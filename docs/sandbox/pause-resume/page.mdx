...
The writable RootFS is a block-COW filesystem. `/tmp`, `/var/tmp`, workspace
paths, and other ordinary RootFS paths persist after a successful checkpoint.
Virtual mounts such as `/proc`, `/dev`, and `/sys` are recreated for each
runtime generation. Do not treat process state under virtual filesystems as
durable data.
