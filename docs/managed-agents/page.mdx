...
| Agent harness | Runtime adapter that runs the session inside a Sandbox0 sandbox |
...
Managed Agents run the selected agent harness inside the per-session Sandbox0 sandbox:
...
Choose the harness by attaching an LLM vault with `sandbox0.managed_agents.engine`.
...
flowchart TD
    ...
    runtime --> wrapper[agent harness wrapper]
    wrapper --> callbacks[Signed runtime callbacks]
    callbacks --> gateway
    wrapper --> llm[model provider API]
    wrapper --> aigateway[optional AI gateway]
    aigateway --> llm
...
6. Sandbox0 claims or resumes the session sandbox and starts the selected agent harness.
...
<CardGroup>
  ...
  <Card title="Skills" href="/docs/managed-agents/skills" cta="Continue">
    Upload custom skill bundles and attach versioned expertise to agents.
  </Card>
