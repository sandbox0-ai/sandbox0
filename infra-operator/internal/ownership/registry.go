...
func Registry() []Entry {
	return []Entry{
		...
		exact("spec.redis.type", "plan", []string{"redis", "global-gateway", "regional-gateway", "cluster-gateway"}, []string{"InfraPlan.Components.EnableRedis"}, UpdateSemanticsDeclarative, "Selects builtin versus external Redis reconciliation and gateway rate-limit backend injection."),
		prefix("spec.redis.builtin", "redis", []string{"redis", "global-gateway", "regional-gateway", "cluster-gateway"}, nil, UpdateSemanticsDeclarative, "Builtin Redis fields are reconciled by the Redis service and injected into gateway rate-limit config."),
		...
		prefix("spec.redis.external", "redis", []string{"redis", "global-gateway", "regional-gateway", "cluster-gateway"}, nil, UpdateSemanticsDeclarative, "External Redis URL secret is injected into gateway rate-limit runtime config."),
		exact("spec.redis.keyPrefix", "redis", []string{"global-gateway", "regional-gateway", "cluster-gateway"}, nil, UpdateSemanticsDeclarative, "Prefixes keys used by gateway Redis rate limiters."),
		...
		prefix("spec.publicExposure", "plan", []string{"global-gateway", "regional-gateway", "cluster-gateway", "manager"}, nil, UpdateSemanticsDeclarative, "Shared public-exposure settings consumed by gateway and manager runtime config."),
		exact("spec.publicExposure.rootDomain", "plan", []string{"global-gateway", "regional-gateway", "cluster-gateway", "manager"}, nil, UpdateSemanticsDeclarative, "Sandbox service public root domain feeds generated external URLs and callbacks."),
		exact("spec.publicExposure.runRootDomain", "plan", []string{"global-gateway", "regional-gateway", "cluster-gateway"}, nil, UpdateSemanticsDeclarative, "Run public root domain feeds production Run URLs and host routing."),
		exact("spec.publicExposure.regionId", "plan", []string{"global-gateway", "regional-gateway", "cluster-gateway", "manager"}, nil, UpdateSemanticsDeclarative, "Public DNS-safe region label feeds generated external URLs and callbacks."),
		exact("spec.publicExposure.runStartupTimeoutSeconds", "plan", []string{"regional-gateway", "cluster-gateway"}, nil, UpdateSemanticsDeclarative, "Run cold-start health wait timeout is injected into production Run host routing."),
