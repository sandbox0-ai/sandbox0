...
func Registry() []Entry {
	return []Entry{
		...
		prefix("spec.sandboxObservability", "sandbox-observability", []string{"sandbox-observability", "cluster-gateway", "manager", "storage-proxy", "netd", "status"}, []string{"InfraPlan.Components.EnableSandboxObservability"}, UpdateSemanticsDeclarative, "Region-local per-sandbox historical observability backend, storage retention, producer ingestion, and user-facing query APIs."),
		prefix("spec.sandboxObservability.builtin", "sandbox-observability", []string{"sandbox-observability", "cluster-gateway", "netd", "status"}, nil, UpdateSemanticsMixed, "Builtin ClickHouse fields are reconciled by the sandbox-observability service and injected into sandbox observability-capable services."),
		prefix("spec.sandboxObservability.external", "sandbox-observability", []string{"cluster-gateway", "netd", "status"}, nil, UpdateSemanticsDeclarative, "External ClickHouse DSN secret is injected into the per-sandbox observability query backend."),
