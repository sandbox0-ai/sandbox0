...
<Callout>
Route rate limits use the gateway rate limit backend. Self-hosted deployments default to the in-memory backend for simple onboarding; configure the gateway `rate_limit_backend` as `redis` when multiple gateway replicas need shared limits.
</Callout>
