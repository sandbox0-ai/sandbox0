...
Every idle and active sandbox Pod must use the external `sandbox0` containerd snapshotter from its first image pull. Warm resume reuses the already unpacked template snapshot. Cold resume first starts the canonical digest-pinned base as a carrier image so scheduling and base unpack finish on the selected node, then activates the same node-local marker path. Configure the RuntimeClass name and its containerd handler in one place:
...
`infra-operator` does not edit or restart host containerd. Before rolling out manager, configure every eligible sandbox node so the handler selects `snapshotter = "sandbox0"` and the proxy socket is registered. The self-describing marker does not depend on CRI snapshot-annotation propagation. `ctld` also needs read/write access to the containerd API socket: after manager selects a node, `ctld` installs the marker, config, and manifest in the `k8s.io` namespace and confirms that CRI can resolve the resulting local image. The generated DaemonSet already mounts the standard containerd socket read/write; custom security policies or socket paths must preserve that access. Containerd 1.x with config version 2 uses:
...
The snapshotter does not maintain a second full-object replay cache. It keeps only containerd overlay metadata on the node, and immutable file extents are read from object storage by range. A cached head is mounted only while it has a live container snapshot: the first `Prepare` or `View` acquires the FUSE mount, removing the last child releases it, and process recovery remounts only heads with live children. Writable overlay removal is asynchronous: containerd makes the snapshot key reusable first, then calls snapshotter cleanup to reclaim detached directories outside Pod teardown. Size node ephemeral storage for that short cleanup overlap and keep containerd garbage collection enabled. Node-local marker images are CRI-managed but not pinned, so normal kubelet/containerd image GC reclaims heads after their Pods stop using them.

The marker image lifecycle does not require a rootfs-head repository or registry credentials. Object storage remains the durable source for the head and OCI envelope. A cold node may still pull the canonical template base image if it is absent locally; a warm claim does not contact a registry for its marker head. Roll out the containerd configuration and healthy `ctld`/snapshotter DaemonSets on every eligible node before updating manager. The DaemonSet uses `OnDelete` updates because replacing a process that owns live FUSE connections can disrupt running sandboxes. Drain the node before deleting or upgrading that Pod.
