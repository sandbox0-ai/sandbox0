...
- If `warmProcesses` are configured, procd readiness stays unavailable until every warm process is running.
...
In this model, the pod only counts as warm-pool capacity after procd has started the warm process and `/readyz` is healthy.
