...
| `resources.ephemeralStorage` | string | `512Mi` | Temporary writable-layer and container-log storage for the sandbox. Increase this for workloads that write heavily to `/tmp` or image-local paths; use Sandbox Volumes for durable data. |
...
| `pod.emptyDirMounts` | Ephemeral Kubernetes `emptyDir` mounts for system templates, such as `/var/lib/docker` for Docker-in-Docker runtime state. |
