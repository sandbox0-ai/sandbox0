...
State-changing Sandbox API requests always require canonical acknowledgement for the attempt before execution, regardless of this setting. Cluster-gateway then buffers the downstream response until the signed result has durable custody. With `durable_async`, an fsync to the cluster-gateway delivery PVC is sufficient to return the buffered response and ClickHouse visibility follows through replay. With `canonical_sync`, the response also waits for ClickHouse to acknowledge the result. If canonical admission fails after the attempt is durably buffered, cluster-gateway does not execute the downstream handler and attempts to append a correlated signed `failed` result with HTTP `503`, `execution_started=false`, `operation_executed=false`, `failure_stage=canonical_audit_admission`, and `failure_code=canonical_ack_unavailable`. If that failed-admission result cannot reach either the delivery buffer or the canonical fallback, the response reports `audit_result=unrecorded`, and the surviving orphan attempt is an audit-completeness incident that requires investigation. After execution, failure of both result-custody paths also returns `503` because the operation may have completed without a durable result fact.

During a ClickHouse outage, `durable_async` traffic can continue only while the local delivery buffer remains writable and durable; audit readers will lag until replay succeeds, with no fixed visibility bound during the outage. If the buffer cannot accept an event, Sandbox0 tries a synchronous canonical fallback and fails closed when neither path can take custody. Mutation admission always fails closed on a canonical timeout or outage because execution cannot start without the attempt ACK. If ClickHouse becomes unavailable after that ACK, `durable_async` can still complete the response after the result is fsynced, while `canonical_sync` returns `503` and retains the result for replay. Choose `canonical_sync` when immediate canonical result visibility is worth the added latency and availability coupling; choose `durable_async` when durable result capture with eventual canonical visibility is the preferred default.
