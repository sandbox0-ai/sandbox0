...
spec:
    ...
    mainContainer:
        image: registry.sandbox0.svc.cluster.local:5000/my-ds-env:v2.0
        resources:
            cpu: "2"
            memory: 4Gi
        env:
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mode` | string | — | Required when `network` is set. Allowed values: `allow-all`, `block-all`. |
| `egress.allowedDomains` | string[] | — | Wildcard-supported domain allowlist (e.g., `*.github.com`). Only used when `mode` is `block-all`. |
| `egress.deniedDomains` | string[] | — | Domain blocklist. |
| `egress.allowedCidrs` | string[] | — | CIDR allowlist (e.g., `10.0.0.0/8`). |
| `egress.deniedCidrs` | string[] | — | CIDR blocklist. |
| `egress.allowedPorts` | array | — | Port/protocol allowlist. `port` must be `1..65535`. `protocol` is optional but must be `tcp` or `udp` if set. Supports optional `endPort` range. |
