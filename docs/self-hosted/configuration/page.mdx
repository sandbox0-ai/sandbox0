...
### Experimental Shared Carrier Pool

`services.manager.config.sharedCarrierPool` enables one platform-owned warm pool for the data-plane cluster. User template images are resolved to immutable revisions and imported into S0FS before they become claimable. A compatible template takes a unique pre-created carrier Pod; a template with an unsupported Pod shape, or a claim made while the pool is empty, uses a cold S0FS carrier instead of creating another warm pool.
...
When enabled, template `pool.minIdle` and `pool.maxIdle` are deprecated compatibility fields and do not own S0FS carrier capacity. The operator derives `carrierImageRef` as `<infra-image-repository>:carrier-base-v1`; override it only when you publish the tiny carrier-base artifact in another repository. The init waiter and importer primer continue to use the manager infra image, because the carrier base intentionally contains no shell.
