...
flowchart TB
    ...
    subgraph cold["Cold path (no idle pod)"]
        slot["Wait for coldStartConcurrency slot"]
...
3. If no idle pod exists (cold claim), the manager waits for a `coldStartConcurrency` slot and then directly creates a new active pod (`createNewPod`)
...
| `maxIdle` | integer | Maximum healthy idle pod count. Template status reports `PoolHealthy=False` if idle pods exceed this value, and cleanup removes excess idle pods. |
...
`maxIdle` puts an upper bound on healthy idle pool size:

- Works with template status conditions (`PoolHealthy=False`) to signal idle over-provisioning
- Allows cleanup to remove excess idle pods
