...
By default, manager uses `encrypted_pg`: credential source specs are encrypted before PostgreSQL persistence, and infra-operator mounts a stable per-installation encryption key into manager. During upgrade, manager backfills legacy plaintext rows to encrypted PostgreSQL rows on startup.
