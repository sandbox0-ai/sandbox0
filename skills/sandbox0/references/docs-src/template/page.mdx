...
Use `startupProbe` for slow-starting sidecars. Sandbox0 owns sandbox pod readiness, and a pod that is merely `Running` but not `Ready` is not counted as available pool capacity.
