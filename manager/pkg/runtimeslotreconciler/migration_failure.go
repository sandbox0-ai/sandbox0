...
type migrationFailureCompletionStore interface {
	GetNomadMigrationFailureForSlot(context.Context, string) (bool, *protocol.MigrationFailureFinalizationReceipt, error)
	CompleteNomadSandboxMigrationFailure(context.Context, string, protocol.MigrationSourceGCRequest, protocol.MigrationSourceGCAcknowledgement) (*sandboxstore.RuntimeSlot, error)
}

// A failed target keeps the ordinary durable reconciliation queue, but cannot
...
func (r *Reconciler) reconcileMigrationFailure(ctx context.Context, store migrationFailureCompletionStore, slot *sandboxstore.RuntimeSlot, receipt *protocol.MigrationFailureFinalizationReceipt) (bool, error) {
	if receipt == nil {
		return false, nil
	}
	if receipt.Validate() != nil {
		return false, errors.New("failed migration finalization is invalid")
	}
	...
	if c.SlotID != slot.ID || c.AllocationID != slot.AllocationID || c.ClusterID != slot.ClusterID || c.NodeID != slot.NodeID ||
		...
		return false, errors.New("failed migration finalization changed physical custody")
	}
	...
	if err := r.allocation.Purge(ctx, AllocationPurgeRequest{OperationID: operationIDs(slot).purge, Target: target}); err != nil {
		return false, fmt.Errorf("purge failed migration allocation: %w", err)
	}
	...
	if !ok {
		return false, errors.New("failed migration allocation acknowledgement is unavailable")
	}
	request := protocol.MigrationSourceGCRequest{Target: receipt.Request.Request.Failure.Request.Restore.Image.Target,
	...
	if terminal == nil || terminal.ID != slot.ID || terminal.State != sandboxstore.RuntimeSlotStateTerminal || terminal.TerminalReason != "migration_failed" ||
		terminal.ResourceLease != c.Resources || terminal.ResourceLeaseState != sandboxstore.RuntimeResourceLeaseReleased {
		return false, errors.New("failed migration authority did not release the exact carrier")
	}
	return true, nil
}
