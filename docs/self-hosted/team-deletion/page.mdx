...
Historical metering delivery records and the derived v3 rootfs opaque-prefix attribution mapping are retained and do not block deletion. The mapping contains no rootfs object reachability or lifecycle truth; it only lets late object-store request windows resolve to the same historical team identity.
