...
`storageKind` defaults to `encrypted_pg`. The manager encrypts credential source specs before storing them in PostgreSQL, and infra-operator mounts a stable per-installation encryption key into manager. During upgrade, manager backfills legacy plaintext rows to `encrypted_pg` on startup.

Use `storageKind: hashicorp_vault` when credential material should live in a Vault-compatible backend. Infra-operator supports builtin OpenBao and external HashiCorp Vault:
...
For `hashicorp_vault`, send `spec` to let manager write the secret to KV v2, or send `externalRef` without `spec` to reference material that already exists in Vault.
...
### Create A Vault-Backed Source

```yaml
name: github-source
resolverKind: static_headers
storageKind: hashicorp_vault
spec:
    staticHeaders:
        values:
            token: ghp_example
...
To reference an existing Vault KV v2 entry instead of writing the secret through Sandbox0:
