...
3. `fullmode-netd` and `fullmode-ctld` run on `gvisor-lite`
...
High-density nodes can opt into fixed, recoverable ctld FUSE shards after booting a compatible kernel:
...
Use this only after draining and upgrading a canary node with a build from the maintained [`sandbox0-ai/linux` stable patch queue](https://github.com/sandbox0-ai/linux/tree/sandbox0/linux-6.18.y-fuse-recovery). The same architecture-specific kernel package can be used across cloud providers; it does not contain provider-specific code. `requireFUSERecovery` is mandatory for fixed shards, and ctld fails closed unless recovery, resend, recovered-request status, cache-domain invalidation, and empty-domain registration are available. See [Self-hosted Configuration](/docs/self-hosted/configuration) for lifecycle, capacity metrics, and shard-count constraints.
