...
- understand how `ttl`, `auto_resume`, public gateway routes, and SSH interact with pause state
...
- public gateway routes configured with `resume: true`
- SSH access through Sandbox0 SSH Gateway

See [Gateway Policy](/docs/sandbox/gateway-policy) and [SSH](/docs/sandbox/ssh) for the user-facing flows.
...
4. resume on the next user action, SSH session, or public gateway route hit
