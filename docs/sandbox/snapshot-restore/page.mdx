...
The default `/tmp` tmpfs capacity is half the sandbox memory limit. Writes
consume the sandbox's memory budget; the capacity itself reserves no memory.
A template can set an explicit `/tmp` capacity using `ephemeralMounts`.
