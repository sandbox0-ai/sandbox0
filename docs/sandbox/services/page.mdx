...
Service runtime supports `cmd`, `warm_process`, and `manual`. Use `cmd` when Sandbox0 should create a new command context for the service. Use `warm_process` when the template already starts a long-lived process and the service should bind to that existing context.
