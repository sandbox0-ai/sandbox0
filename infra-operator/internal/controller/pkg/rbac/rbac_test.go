...
func TestReconcileManagerRBACIncludesRuntimeClassReadPermission(t *testing.T) {
	scheme := runtime.NewScheme()
	require.NoError(t, corev1.AddToScheme(scheme))
	require.NoError(t, rbacv1.AddToScheme(scheme))
	require.NoError(t, infrav1alpha1.AddToScheme(scheme))
	...
	found := false
	for _, rule := range role.Rules {
		if !contains(rule.APIGroups, "node.k8s.io") {
			continue
		}
		if !contains(rule.Resources, "runtimeclasses") {
			continue
		}
		assert.ElementsMatch(t, []string{"get", "list", "watch"}, rule.Verbs)
		found = true
	}
	assert.True(t, found, "expected manager cluster role to include RuntimeClass read permission")
}
