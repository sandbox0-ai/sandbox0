...
In practice, “available” means the pod is ready for claiming, not merely `Pod Running`. The writable rootfs is initialized before runtime activation and is captured by later rootfs checkpoints.
...
- Readiness is based on the sandbox container and procd state
