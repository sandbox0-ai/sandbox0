...
The snapshotter does not maintain a second full-object replay cache. It keeps only containerd overlay metadata on the node, and immutable file extents are read from object storage by range. A cached head is mounted only while it has a live container snapshot: the first `Prepare` or `View` acquires the FUSE mount, removing the last child releases it, and process recovery remounts only heads with live children. Its DaemonSet uses `OnDelete` updates because replacing a process that owns live FUSE connections can disrupt running sandboxes. Drain the node before deleting or upgrading that Pod.
