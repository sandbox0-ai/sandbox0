...
func TestBuildRuntimeAssetsUsesCanonicalNetworkConfig(t *testing.T) {
	...
	infra.Spec.Network.Config = &infrav1alpha1.NetworkRuntimeConfig{
		NodeName:       "node-a",
		MetricsPort:    19091,
		HealthPort:     18081,
		ProxyHTTPPort:  28080,
	...
	if !strings.HasPrefix(assets.ConfigRef.ConfigMapName, infra.Name+"-ctld-networking-") {
		t.Fatalf("config map name = %q, want ctld networking prefix", assets.ConfigRef.ConfigMapName)
	}
	...
	persisted := &apiconfig.NetworkRuntimeConfig{}
...
func TestBuildRuntimeAssetsMigratesLegacyManagedMITMCA(t *testing.T) {
	infra := newRuntimeTestInfra()
	legacy := newRuntimeTestMITMCASecret(t, infra, legacyManagedMITMCASecretName(infra.Name))
	legacyCert := append([]byte(nil), legacy.Data[mitmCACertKey]...)
	legacyKey := append([]byte(nil), legacy.Data[mitmCAKeyKey]...)

	assets, client := buildRuntimeAssets(t, infra, legacy)
	secretName := managedMITMCASecretName(infra.Name)
	assertRuntimeMITMCA(t, assets, secretName)
	...
	if err := client.Get(context.Background(), types.NamespacedName{
		...
		t.Fatalf("get migrated MITM CA: %v", err)
	}
	if !reflect.DeepEqual(migrated.Data[mitmCACertKey], legacyCert) || !reflect.DeepEqual(migrated.Data[mitmCAKeyKey], legacyKey) {
		t.Fatal("migrated MITM CA did not preserve the legacy trust root")
	}
}
