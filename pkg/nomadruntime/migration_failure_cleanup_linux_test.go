...
func TestMigrationFailureCleanupRetainsImageAndExternalCrashJournal(t *testing.T) {
	for _, attached := range []bool{false, true} {
		t.Run(map[bool]string{false: "canceled-unattached", true: "consumed-attached"}[attached], func(t *testing.T) {
			d, observation, _, runner := migrationRestoreNodeFixture(t, func(d *nodeRuntime, image protocol.MigrationImagePrepareRequest) {
			...
			c.NodeUID = d.nodeUID
			c.OperationID = protocol.MigrationFailureCleanupOperationID(failure.Restore.Image.Publication.Assignment.OperationID)
			...
			record, err = d.journal.Get(c.SlotID)
			require.NoError(t, err)
			require.Equal(t, runtimeSlotFailureCleanupJournalVersion, record.Version)
			...
			record, err = d.journal.Get(c.SlotID)
			require.NoError(t, err)
			require.Equal(t, runtimeSlotFailureFinalizeJournalVersion, record.Version)
			...
			record, err = d.journal.Get(c.SlotID)
			require.NoError(t, err)
			require.Equal(t, runtimeSlotFailureGCJournalVersion, record.Version)
			require.NoError(t, reopened.Close())
			reopened, err = newRuntimeSlotJournal(path, time.Hour)
			require.NoError(t, err)
			d.journal = reopened
