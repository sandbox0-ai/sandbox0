...
const (
	ConfigPath           = "/config/ctld-networking.yaml"
	ConfigVolumeName     = "ctld-networking-config"
	RunVolumeName        = "ctld-networking-run"
	RunMountDirectory    = "/run"
	ConfigHashAnnotation = "infra.sandbox0.ai/ctld-networking-config-hash"
	// Keep the existing host directory for this rollout so durable audit
	// records created by predecessor ctld Pods are replayed instead of lost.
	legacyAuditSpoolHostRoot = "/var/lib/sandbox0/netd"
)
...
func (r *Reconciler) BuildRuntimeAssets(ctx context.Context, compiledPlan *infraplan.InfraPlan) (*RuntimeAssets, error) {
	...
	name := fmt.Sprintf("%s-ctld-networking", scope.Name)
	labels := common.GetServiceLabels(scope.Name, "ctld")
	config := &apiconfig.NetworkRuntimeConfig{}
	...
	if err := redissvc.ApplyNetworkRuntimeRedisConfig(ctx, r.Resources.Client, scope.Owner(), config); err != nil {
		return nil, err
	}
	...
	if err := meteringsvc.ApplyNetworkRuntimeConfig(ctx, r.Resources.Client, scope.Owner(), config); err != nil {
		return nil, err
	}
	if config.EgressAuthResolverURL == "" {
		config.EgressAuthResolverURL = compiledPlan.Network.EgressAuthResolverURL
	}
	if err := sandboxobssvc.ApplyNetworkRuntimeConfig(ctx, r.Resources.Client, scope.Owner(), compiledPlan.Services.ClusterGateway.URL, config); err != nil {
		return nil, err
	}
	...
	auditKeySecretName, auditPrivateKeyKey, _ := compiledPlan.NetworkAuditKeyRefs()
	if mitmCASecretName != "" {
	...
	if config.SandboxObservabilityIngestURL != "" {
		volumes = append(volumes,
			corev1.Volume{
				Name: "audit-spool",
				VolumeSource: corev1.VolumeSource{HostPath: &corev1.HostPathVolumeSource{
					Path: filepath.Join(legacyAuditSpoolHostRoot, scope.Namespace, scope.Name), Type: &directoryOrCreate,
				}},
			},
			corev1.Volume{
		...
		volumeMounts = append(volumeMounts,
			corev1.VolumeMount{Name: "audit-spool", MountPath: "/var/lib/sandbox0/ctld/networking"},
