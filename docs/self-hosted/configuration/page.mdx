...
Active Nomad network updates use a PostgreSQL desired/apply/ack transaction bound to the exact slot revision, allocation, node boot, claim, network namespace, current policy digest, and requested policy bytes. Manager leaves the published sandbox config unchanged while it sends the deterministic operation through the authenticated node channel. It publishes the new config and runtime-slot policy digest atomically only after ctld returns a matching applied-policy token; a lost API or node response is retried from the same durable operation by every manager rollout. Concurrent different updates conflict, while an exact retry is idempotent. Pause, deletion, hard expiry, lifecycle work, slot replacement, or revision drift cancels pending policy instead of applying it to another incarnation. Nomad credential-binding updates remain fail closed until credential projection and cleanup are available.
