...
Before a cold S0FS open exposes a committed state, the runtime verifies that every referenced non-inline segment exists in object storage. A missing object fails the open with an integrity error instead of exposing a mount that can later return partial `EIO` reads.
