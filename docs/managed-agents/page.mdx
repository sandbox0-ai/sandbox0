...
| Sandbox as tool | A resident runtime service owns the agent loop and uses Sandbox0 as an isolated tool target. | `openai-agents`, `pi` |
...
flowchart TD
    ...
    resident --> aigateway[optional AI gateway]
    aigateway --> llm
...
- `claude` and `codex` run agent in sandbox; `openai-agents` and `pi` use sandbox as tool.
