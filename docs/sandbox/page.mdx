...
State-changing Sandbox API operations always require ClickHouse to acknowledge the attempt before execution, regardless of the configured delivery mode. Cluster-gateway buffers the HTTP response until the signed result has durable custody. In `durable_async`, an fsync to the cluster-gateway delivery PVC is sufficient to return that response and the result reaches ClickHouse through replay; in `canonical_sync`, the response also waits for the result ACK. If canonical admission fails after the attempt is durably buffered, the downstream operation is not executed and cluster-gateway attempts to append a correlated signed `failed` result with HTTP `503`, `execution_started=false`, `operation_executed=false`, `failure_stage=canonical_audit_admission`, and `failure_code=canonical_ack_unavailable` for replay. If terminal-result custody fails in both the spool and canonical fallback, the response reports `audit_result=unrecorded`; after execution this means the operation may have completed without a durable result fact. A `5xx` or panic result after execution is recorded as `unknown`, because the downstream operation may have completed even if the gateway did not receive a definitive response.
