...
State-changing Sandbox API requests are always strict regardless of this setting. Cluster-gateway requires canonical acknowledgement for the attempt before execution, buffers the downstream response, and requires canonical acknowledgement for the signed result before returning that response. The delivery buffer is still used to preserve a pending fact for replay, but it never turns a mutation into a successful response before ClickHouse acknowledges the fact. If canonical admission fails after the attempt is durably buffered, cluster-gateway does not execute the downstream handler and appends a correlated signed `failed` result with HTTP `503`, `execution_started=false`, `operation_executed=false`, `failure_stage=canonical_audit_admission`, and `failure_code=canonical_ack_unavailable`; both facts become query-visible after replay.
