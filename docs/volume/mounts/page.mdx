...
Operator-managed deployments keep one active and one synchronized standby ctld process on each sandbox node. The standby holds a second channel for every kernel FUSE connection and the backend recovery state needed to take over without unpublishing the portal. This protects existing mounts from a single ctld process or Pod failure; it does not preserve mounts across a node reboot or simultaneous loss of both processes.
