...
func TestRootFSImportLayoutPublicationSelectionAndOldWorkerFenceIntegration(t *testing.T) {
	ctx := t.Context()
	pool := newSandboxStoreIntegrationPool(t)
	store := NewPGSandboxStore(pool)
	legacyBegin, legacyResult, legacyRef := layoutImportFixture(t, "layout-same-inputs", "")
	...
	begin, result, ref := layoutImportFixture(t, "layout-same-inputs", rootfsimporter.XFSFileRangesV1)
	...
	_, err = pool.Exec(ctx, `UPDATE manager.rootfs_import_operations SET data_layout_policy='' WHERE operation_id=$1`, op.ID)
	require.ErrorAs(t, err, &pgErr)
	require.Equal(t, "23514", pgErr.Code)
	...
	require.Equal(t, rootfsimporter.XFSFileRangesV1, artifact.ImportDataLayoutPolicy)
	proof, err := rootfsimporter.DecodeReadyArtifactAttestation(artifact.Attestation)
	require.NoError(t, err)
	require.Equal(t, 2, proof.Version)
	...
	policy.ImportDataLayoutPolicy = rootfsimporter.XFSFileRangesV1
	selected, err = store.GetReadyRootFSBaseArtifact(ctx, result.SourceOCIDigest.String(), artifact.Platform, policy)
	require.NoError(t, err)
	...
	policy.ImportDataLayoutPolicy, policy.ImportDataRangeBytes, policy.SourceOCIRef = "", 0, ""
	...
	_, err = pool.Exec(ctx, `UPDATE manager.rootfs_base_artifacts SET data_layout_policy='' WHERE artifact_digest=$1`, artifact.ArtifactDigest)
	require.ErrorAs(t, err, &pgErr)
	require.Equal(t, "23514", pgErr.Code)
