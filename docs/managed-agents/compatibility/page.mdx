...
| Custom skills | Supported through uploaded skill versions. See [Skills](/docs/managed-agents/skills). |
...
| Agent harnesses | Supported for `claude` and `codex` |
...
| Environment variable credentials | Stored as sandbox environment placeholders and resolved by Sandbox0 egress auth. They are available to sandbox-backed runtimes such as `claude` and `codex`, not as SDK client configuration secrets. |
| Sandbox claim | Agent harnesses claim or resume a Sandbox0 sandbox before the run. |
...
| Send user message | Starts the selected harness and appends status events. The harness prepares the sandbox before the run. |
