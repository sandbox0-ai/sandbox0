...
func (h *SandboxObservabilityHandler) IngestLogs(c *gin.Context) {
	h.ingestSandboxObservability(c, func() (int, func() error, []zap.Field, error) {
		var req struct {
		...
		return len(req.Logs), func() error {
			return h.writer.InsertLogs(c.Request.Context(), req.Logs)
		}, []zap.Field{zap.Int("log_count", len(req.Logs))}, nil
	}, "too many logs", "failed to ingest sandbox observability logs")
}
...
func (h *SandboxObservabilityHandler) IngestRuntimeSamples(c *gin.Context) {
	h.ingestSandboxObservability(c, func() (int, func() error, []zap.Field, error) {
		...
		return len(req.Samples), func() error {
			return h.writer.InsertRuntimeSamples(c.Request.Context(), req.Samples)
		}, []zap.Field{zap.Int("sample_count", len(req.Samples))}, nil
	}, "too many runtime samples", "failed to ingest sandbox runtime samples")
}

func (h *SandboxObservabilityHandler) ingestSandboxObservability(
	c *gin.Context,
	decode func() (int, func() error, []zap.Field, error),
	...
	count, insert, fields, err := decode()
	if err != nil {
		spec.JSONError(c, http.StatusBadRequest, spec.CodeBadRequest, "invalid request body")
		return
	}
	if count > maxSandboxObservabilityLimit {
		spec.JSONError(c, http.StatusBadRequest, spec.CodeBadRequest, tooManyMessage)
		return
	}
	if err := insert(); err != nil {
		h.writeIngestError(c, err, failureMessage, fields...)
		return
	}
	spec.JSONSuccess(c, http.StatusAccepted, gin.H{"inserted": count})
}

func (h *SandboxObservabilityHandler) watchEvents(c *gin.Context, query sandboxobservability.EventQuery) {
	watchRepo, opts, ok := h.prepareSandboxObservabilityWatch(c, query.EndTime, query.Cursor, query.Limit, query.StartTime)
	if !ok {
		return
	}
	watchSandboxObservability(h, c, opts,
		func(opts sandboxobservability.WatchOptions) (*sandboxobservability.EventListResult, error) {
			return watchRepo.WatchEvents(c.Request.Context(), query, opts)
		},
		func(encoder *json.Encoder, flusher http.Flusher, result *sandboxobservability.EventListResult, opts *sandboxobservability.WatchOptions) bool {
			return h.writeWatchEvents(c, encoder, flusher, result, opts)
		},
		"failed to watch sandbox observability events",
		"failed to watch sandbox observability events",
		[]zap.Field{
			zap.String("sandbox_id", query.SandboxID),
			zap.String("team_id", query.TeamID),
...
func (h *SandboxObservabilityHandler) watchLogs(c *gin.Context, query sandboxobservability.LogQuery) {
	...
	watchSandboxObservability(h, c, opts,
		func(opts sandboxobservability.WatchOptions) (*sandboxobservability.LogListResult, error) {
			return watchRepo.WatchLogs(c.Request.Context(), query, opts)
		},
		func(encoder *json.Encoder, flusher http.Flusher, result *sandboxobservability.LogListResult, opts *sandboxobservability.WatchOptions) bool {
			return h.writeWatchLogs(c, encoder, flusher, result, opts)
		},
		"failed to watch sandbox observability logs",
		"failed to watch sandbox observability logs",
		[]zap.Field{
			zap.String("sandbox_id", query.SandboxID),
			zap.String("team_id", query.TeamID),
...
func (h *SandboxObservabilityHandler) prepareSandboxObservabilityWatch(
	c *gin.Context,
	endTime *time.Time,
	cursor string,
	limit int,
	startTime *time.Time,
) (sandboxobservability.WatchRepository, sandboxobservability.WatchOptions, bool) {
	if !validateSandboxObservabilityWatch(c, endTime) {
		return nil, sandboxobservability.WatchOptions{}, false
	}
	watchRepo, ok := h.watchRepository(c)
	if !ok {
		return nil, sandboxobservability.WatchOptions{}, false
	}
	return watchRepo, buildSandboxObservabilityWatchOptions(cursor, limit, startTime), true
}

func watchSandboxObservability[T any](
	h *SandboxObservabilityHandler,
	c *gin.Context,
	opts sandboxobservability.WatchOptions,
	fetch func(sandboxobservability.WatchOptions) (*T, error),
	writeBatch func(*json.Encoder, http.Flusher, *T, *sandboxobservability.WatchOptions) bool,
	queryErrorMessage string,
	watchErrorMessage string,
	queryFields []zap.Field,
) {
	result, err := fetch(opts)
	if err != nil {
		h.writeQueryError(c, err, queryErrorMessage, queryFields...)
		return
	}
	...
	for {
		fullBatch := writeBatch(encoder, flusher, result, &opts)
		...
		result, err = fetch(opts)
		if err != nil {
			h.writeWatchErrorLine(c, encoder, flusher, err, watchErrorMessage)
			return
		}
	}
}
