...
After quota admission, manager claims a ready idle pod or creates a cold pod directly. By default, warm-pool ReplicaSets scale to each template's configured `minIdle`. In multi-cluster mode, scheduler routing prefers ready idle capacity, then estimated cluster headroom, configured weight, and deterministic fallback order.

### S0FS Rootfs Rollout

S0FS template imports, new-claim admission, and shared warm capacity are independent rollout controls. This lets an operator import images in shadow mode, admit an explicit cohort through cold carriers, and only then enable the single platform-owned warm pool. Existing S0FS sandboxes remain resumable when new admission is switched off.
            ...
            config:
                ...
                s0fsRuntime:
                    ...
                    admission:
                        ...
                        teamIds:
                            - internal-canary-team
                        rejectLegacyClaims: true
```

`templateImageFS.enabled` runs shadow imports even when admission mode is `off`. Its optional `teamIds` and `templateIds` select which templates to import; when both are empty, the existing import-all behavior is preserved. Admission has a separate allowlist: `teamIds` applies only to private templates, while `templateIds` matches the logical template ID in either scope. When both selectors are present, a private template must match both the team and template ID, while a public template still matches by template ID. Empty admission allowlists admit nothing. Set mode to `shared` and `sharedCarrierPool.enabled: true` after the cold cohort is healthy. On a dedicated green data plane, `rejectLegacyClaims: true` prevents an unmatched request from creating a legacy rootfs.

When shared admission is enabled, template `pool.minIdle` and `pool.maxIdle` are deprecated compatibility fields and do not own S0FS carrier capacity. The operator derives the immutable carrier artifact as `<infra-image-repository>:<infra-image-tag>-carrier-base`; override `carrierImageRef` only when you publish that artifact elsewhere. The init waiter and importer primer continue to use the manager infra image, because the carrier base intentionally contains no shell.

Treat this mode as experimental until your production-like cluster demonstrates stable claim and resume latency. A revision that is ready in the control plane but has not reached the selected data plane returns a retryable unavailable response rather than falling back to a legacy image Pod.
...
- v3 rootfs CAS keys use an opaque team hash under `sandbox-rootfs/cow-v3/teams/...`; a retained PostgreSQL mapping resolves that prefix back to the owning team when request windows flush
- v3 rootfs CAS requests use `subject_type=rootfs` and the team ID as `subject_id` because immutable objects can be shared by multiple sandboxes in the same team; they are not falsely assigned to one sandbox
- bucket-wide rootfs maintenance listings that do not identify one team remain `rootfs_unknown` rather than receiving fabricated customer attribution
...
Sandbox Pods use the single `gvisor-rootfs` runtime family. `infra-operator` creates that Kubernetes `RuntimeClass`; the node image or bootstrap process must install `runsc` and configure the matching containerd handler and external snapshotter proxy.
...
The field is fixed to `gvisor-rootfs`; other values are rejected by the operator API. `infra-operator` does not install `runsc`, write `/etc/containerd/config.toml`, or restart containerd. Do that through your node image, node bootstrap automation, or a dedicated privileged node setup process before scheduling sandbox workloads there.
...
Configure containerd so the `gvisor-rootfs` handler points to `runsc` and selects the `sandbox0` external snapshotter proxy:
...
Restart containerd after installing this configuration. The operator-managed rootfs snapshotter DaemonSet publishes the proxy socket. `sandboxRuntimeClassName` applies only to sandbox Pods; do not run ctld or the snapshotter under `gvisor-rootfs`.
...
The external rootfs snapshotter keeps a disposable shared node-local object cache. This cache accelerates metadata lookup and cold lower reads. The default cap is `20Gi` per node. Set `rootfsObjectCacheMaxBytes: "0"` to disable it, or tune the GC fields to fit your node disk:
...
Cache contents are safe to delete. If a cached object is missing, corrupt, too old, or evicted to preserve free space, the snapshotter downloads and verifies it again.

Writable-rootfs I/O has these performance characteristics:

- sandbox writes and reads of upper-layer files use the native node-local overlayfs path, not FUSE
- unchanged files restored from a Head are read lazily; the initial implementation uses 1 MiB chunks, so a cold 4 KiB read can fetch and verify up to 1 MiB before later reads hit the shared disk cache
- a directory lookup loads its index and one hash shard; large `readdir` operations stream shards instead of materializing the complete directory at once
- changing any content in a regular upper-layer file currently makes capture reread and rechunk the complete file; content-addressed chunks avoid storing unchanged chunks again, but do not avoid that local read and hashing cost
- `/tmp`, `/procd`, `/procd-image`, webhook state, Sandbox Volumes, and every other runtime-owned Pod VolumeMount are excluded from rootfs capture; their owning subsystem supplies them after runtime reconstruction
- UNIX domain sockets are treated as process state and become absent in the published Head instead of making capture fail or reappearing as stale socket paths
- read-only access-time changes are not durable rootfs state; use explicit application metadata instead of persisted `atime` as a workflow signal

The snapshotter owns live FUSE mounts and uses an `OnDelete` DaemonSet update strategy. Drain the node before a planned snapshotter Pod replacement. An unexpected process restart cannot reconnect an existing overlay child to the replacement FUSE superblock: the node first becomes not-ready, then publishes the replacement process instance after its full readiness checks pass. Manager detects that instance change, barriers and seals the affected sandbox through the durable rootfs lifecycle, and recreates its runtime. A cold lower read can fail until that reconstruction completes, so this is automatic recovery rather than transparent mount failover.
...
- `storage.runtime.s0fsHotCacheMaxSize` is the shared node budget for conservatively estimated active and detached S0FS engine memory; cold activation reserves the bounded metadata and segment caches before opening state, first evicts detached engines, then fails explicitly if active engines already consume the budget. Detached retention has no age-based expiry, uses probation/protected admission to resist short-lived churn, defaults to `1Gi`, and both retention and active admission enforcement are disabled by setting the value to `0`
...
Monitor S0FS engine residency with `ctld_s0fs_metadata_resident_estimated_bytes` split by `active` and `detached`, plus `ctld_s0fs_metadata_budget_pressure_total`. Resume-cache behavior remains available through `ctld_s0fs_hot_cache_requests_total`, `ctld_s0fs_hot_cache_admissions_total`, `ctld_s0fs_hot_cache_evictions_total`, and the entry, segment, and budget gauges under the `ctld_s0fs_hot_cache_` prefix. Labels are intentionally low-cardinality; sandbox and volume IDs appear only in structured logs.
...
The first open of an encrypted version `1` head after switching to version `2` decrypts the legacy whole-object AEAD payload and rewrites the local head as version `2`. Budget extra memory for that one-time migration; later opens decode chunks directly into the disk-backed metadata index.
