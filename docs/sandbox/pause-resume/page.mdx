...
Every warm sandbox Pod uses the delegating `sandbox0` containerd snapshotter from its first image pull. Normal template layers still use overlayfs. While the sandbox runs, `ctld` asynchronously drains stable writable-upperdir changes into immutable file chunks, file manifests, and hash-sharded directory metadata. A pause briefly barriers `procd`, drains the remaining dirty tail, and seals one complete immutable Merkle head. Unchanged objects are shared with the previous head. Manager then publishes a digest-pinned metadata-only image containing one filesystem-empty marker layer. A content-addressed copy of that tiny marker is kept in the same object inventory so discovery does not depend on CRI annotation propagation or containerd content-GC timing. Neither template layers nor rootfs data chunks are copied into that image.
...
At warm resume, manager adopts a Pod whose digest-pinned template base is already unpacked and replaces only its `procd` image with the metadata head. Containerd keeps the Pod sandbox, recreates `procd`, and the external snapshotter mounts the immutable head lazily above that deterministic base snapshot with a fresh writable overlay. A path lookup reads only the head and the matching directory shard; regular-file chunks are fetched and cached only when read. Rootfs seal and head-attach work therefore depend on the unsynchronized dirty tail and bounded metadata, not total saved rootfs capacity. End-to-end time until the Pod is Ready also includes Kubernetes scheduling, kubelet, image, and runtime work and is measured separately.
...
|-----------|------------------|
| Warm resume | Adopts a warm Pod, swaps only the `procd` image to the digest-pinned metadata head, and waits for that exact image before runtime activation |
| Cold resume | Creates a Pod with the metadata head and an internal image volume that first makes the digest-pinned template base snapshot available on that node |
