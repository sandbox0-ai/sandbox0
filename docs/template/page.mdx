...
The source and target warm-slot classes must use the same attested platform and RootFS format generation. Capture fails closed if the base artifact, source image digest, platform, writer grant, allocation, or node incarnation changes before publication.
</Callout>
