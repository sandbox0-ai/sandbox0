...
<Callout variant="info">
Metering windows are separate usage-ledger records for quota, showback, and billing export. Historical observability tables do not store metering windows, even when the deployment also enables metering on the same region ClickHouse component. Metering is optional and disabled by default; when enabled, it writes usage truth to ClickHouse only. File audit is not part of these endpoints.
</Callout>
