...
1. The manager first tries to claim a Sandbox0-ready idle pod (`claimIdlePod`)
...
In practice, “available” means the sandbox readiness gate is true, not merely `Pod Running`.
...
Warm-pool capacity is counted from idle pods that reach Sandbox0-managed readiness.
...
- Sandbox0 owns the final readiness gate for sandbox pods. Sidecar `readinessProbe` is allowed, but Sandbox0 uses it as one input to the sandbox-managed readiness decision.

If a template uses a sidecar to host a warm process, use `readinessProbe` on the sidecar and let Sandbox0 drive the final pod readiness gate:
...
In this model, the pod only counts as warm-pool capacity after Sandbox0 sees the sidecar readiness signal and marks its readiness gate ready.
