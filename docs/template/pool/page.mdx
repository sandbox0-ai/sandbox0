...
2. If successful (hot claim), the pod is relabeled to `active`, detached from the ReplicaSet, and returned after its quota handoff is committed; pool replenishment happens asynchronously via ReplicaSet reconcile
...
|-------|------|-------------|
| `minIdle` | integer | Minimum number of idle pods to maintain (ReplicaSet replicas). Admitted claims avoid a cold start while ready idle pods are available. |
| `maxIdle` | integer | Compatibility ceiling for the pool contract. The current manager does not autoscale above `minIdle`; template status still reports `PoolHealthy=False` if idle pods exceed `maxIdle`. |
...
For a team-owned template, Team Quota temporarily reserves one selected idle runtime during the cross-resource hot-claim handoff. This prevents a ReplicaSet replacement race from exceeding the team's regional capacity. The reserve is released when ownership commits, but the claim requires that temporary capacity headroom; see [Team Quota](/docs/sandbox/self-hosted/team-quota#enforcement-matrix).
