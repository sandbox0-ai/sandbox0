...
| Core dependencies | `spec.database`, `spec.metadataDatabase`, `spec.storage`, `spec.registry` | PostgreSQL, s0fs metadata, object storage, registry integration |
...
- `services.storageProxy.config.objectEncryptionEnabled` enables application-layer encryption for S0FS object storage and node-local volume cache files
