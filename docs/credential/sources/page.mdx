...
Sandbox0 supports encrypted PostgreSQL storage and Vault-compatible storage backends. API users keep sending the same write-only `spec` in both modes. For self-hosted backend configuration, see [Self-hosted Configuration](/docs/self-hosted/configuration#credential-source-storage).
