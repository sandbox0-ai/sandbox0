...
In single-cluster fullmode, the storage API runtime runs in the `manager` Deployment. Each sandbox node has `ctld-a` and `ctld-b` DaemonSet Pods; the elected primary continuously persists writable-rootfs changes and seals metadata heads while owning volume portals and network policy, and the synchronized standby assumes those responsibilities after promotion. A separate external containerd snapshotter lazily serves immutable metadata heads from S3/OSS object ranges. PostgreSQL, S3/OSS, and the optional image registry are middleware dependencies.
