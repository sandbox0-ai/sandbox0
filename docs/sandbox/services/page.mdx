...
- Route rate limits are enforced by a distributed, fail-closed Sandbox Service abuse guard backed by `Sandbox0Infra.spec.redis`. This guard is independent from Team Quota and uses its own Redis namespace and metrics.
