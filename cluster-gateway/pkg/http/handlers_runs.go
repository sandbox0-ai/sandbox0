...
func (s *Server) prepareAndDeployRun(ctx context.Context, authCtx *gatewayauthn.AuthContext, runIDOrSlug string, req runs.RunDeployRequest) (*runs.RunDeployResult, error) {
...
func (s *Server) compileRunRevision(ctx context.Context, authCtx *gatewayauthn.AuthContext, req runs.RunDeployRequest) (runs.RunRevisionSpec, runs.RunSource, error) {
	...
	case "", runs.RevisionSourceSnapshot:
		...
		return specValue, runs.RunSource{Type: runs.RevisionSourceSnapshot, Snapshot: &runs.SnapshotRevisionSource{SnapshotIDs: snapshotIDs}}, nil
	default:
		return runs.RunRevisionSpec{}, runs.RunSource{}, fmt.Errorf("unsupported source type %q", req.Source.Type)
	}
}

func (s *Server) compileSandboxServiceRevision(ctx context.Context, authCtx *gatewayauthn.AuthContext, source runs.SandboxServiceSource, slug string) (runs.RunRevisionSpec, runs.RunSource, error) {
	...
	for _, mount := range sandbox.Mounts {
		...
		mounts = append(mounts, runs.RunRevisionMount{SnapshotID: snapshot.Id, MountPath: mount.MountPoint, ReadOnly: true})
	}
	...
	return specValue, runs.RunSource{Type: runs.RevisionSourceSandboxService, SandboxService: &runs.SandboxServiceSource{SandboxID: sandboxID, ServiceID: serviceID}}, nil
}

func (s *Server) createRunSourceSnapshot(ctx context.Context, authCtx *gatewayauthn.AuthContext, volumeID, name string) (*apispec.Snapshot, error) {
	var snapshot apispec.Snapshot
	body := map[string]any{"name": name}
	path := "/sandboxvolumes/" + url.PathEscape(volumeID) + "/snapshots"
	if err := s.doRunJSON(ctx, s.cfg.StorageProxyURL, internalauth.ServiceStorageProxy, http.MethodPost, path, authCtx.TeamID, authCtx.UserID, append(authCtx.Permissions, gatewayauthn.PermSandboxVolumeWrite), body, &snapshot); err != nil {
...
func (s *Server) materializeRunVolume(ctx context.Context, teamID, userID string, snapshotID string) (string, error) {
	...
	if err := s.doRunJSON(ctx, s.cfg.StorageProxyURL, internalauth.ServiceStorageProxy, http.MethodPost, "/sandboxvolumes", teamID, userID, []string{gatewayauthn.PermSandboxVolumeCreate}, body, &volume); err != nil {
