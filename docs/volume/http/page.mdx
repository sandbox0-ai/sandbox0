...
Requests go through the normal gateway chain and team-scoped auth. For `s0fs` Volumes, the storage runtime in manager lazily attaches the Volume and reclaims idle direct mounts later. For `s3` backend Volumes, direct file requests are supported only while an active ctld mount owner can serve the request. See [Volume Backends](/docs/sandbox/volume/backends).
</Callout>
...
<Callout variant="info">
The client helpers below call the same `SandboxVolume file` HTTP endpoints documented on this page. They do not require mounting an `s0fs` Volume into a Sandbox first.
</Callout>
...
summary, err := client.UploadVolumeDirectory(ctx, volumeID, "./dist", "/apps/current")
if err != nil {
    log.Fatal(err)
}
fmt.Printf("uploaded files=%d bytes=%d\\n", summary.Files, summary.Bytes)
...
summary = client.volumes.upload_directory(volume_id, "./dist", "/apps/current")
print(f"uploaded files={summary.files} bytes={summary.bytes_}")
...
const archive = await readFile("dist.tar");
const summary = await client.volumes.importArchive(volumeId, "/apps/current", archive);
console.log("uploaded files=", summary.files, "bytes=", summary.bytes);
...
s0 volume files upload vol_abc123 ./dist /apps/current --recursive
  ...
  tabs={[
    {
      label: "Go",
      language: "go",
      code: `summary, err := client.UploadVolumeDirectory(ctx, volumeID, "./dist", "/apps/current")
if err != nil {
    log.Fatal(err)
}

fmt.Printf("uploaded files=%d dirs=%d symlinks=%d bytes=%d\\n",
    summary.Files,
    summary.Directories,
    summary.Symlinks,
    summary.Bytes,
    ...
    },
    {
      label: "Python",
      language: "python",
      code: `summary = client.volumes.upload_directory(volume_id, "./dist", "/apps/current")

print(
    f"uploaded files={summary.files} "
    f"dirs={summary.directories} "
    f"symlinks={summary.symlinks} "
    f"bytes={summary.bytes_}"
)`
    },
    {
      label: "TypeScript",
...
console.log("uploaded files=", summary.files, "bytes=", summary.bytes);`
    },
    {
      label: "CLI",
      language: "bash",
      code: `s0 volume files upload vol_abc123 ./dist /apps/current --recursive`
    }
  ]}
/>
...
<CardGroup>
  ...
  <Card title="Fork" href="/docs/sandbox/volume/fork" cta="Continue">
