...
func configureCarrierPool(cfg *config.ManagerConfig, store carrierpool.Store) (*carrierpool.Worker, error) {
	...
	for _, v := range c.PrewarmWindows {
		...
		windows = append(windows, carrierpool.PrewarmWindow{Name: v.Name, Start: v.Start, End: v.End, RepeatWeekly: v.RepeatWeekly, Slots: v.Slots, CPUMillicores: v.CPUMillicores, MemoryBytes: v.MemoryBytes, SecurityClass: class})
	}
