...
| Resolver kind | Required spec field | Typical use |
|--------------|---------------------|-------------|
| `static_headers` | `spec.staticHeaders.values` | Bearer tokens and header fragments |
| `static_tls_client_certificate` | `spec.staticTLSClientCertificate` | mTLS client authentication |
| `static_username_password` | `spec.staticUsernamePassword` | Username/password based outbound auth |
...
fmt.Println(source.Name)`
    },
    {
      label: "Python",
      language: "python",
      code: `from sandbox0.apispec.models.credential_source_write_request import CredentialSourceWriteRequest
...
After creating a source, bind it into a sandbox or template with [`credentialBindings`](/docs/credential/bindings).
