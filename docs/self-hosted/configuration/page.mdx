...
Sandbox0 platform services emit logs to container stdout and expose Prometheus-compatible metrics on their metrics ports. Configure `spec.observability` when you want the operator to wire sandbox0-owned platform components into an observability backend.

Observability is disabled by default. The operator owns collection integration only: it can install collectors and a local builtin backend, or point platform services at an external collector. User-facing sandbox observability APIs, query adapters, dashboards, and retention policy beyond the selected backend remain outside this layer.
...
The builtin backend installs an OpenTelemetry Collector and a single-node ClickHouse instance. Use it for local self-hosted environments and small deployments where a built-in collection target is enough.

If your platform already has an observability stack, use `external`. In `existingCollector` mode sandbox0 does not install collectors; it only injects standard OTEL trace environment variables into platform services:
...
The legacy direct trace exporter config remains supported and overrides backend-derived trace endpoint defaults:
