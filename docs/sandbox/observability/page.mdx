...
On Nomad nodes, ctld samples stock `runsc` only after matching the current writer and carrier journals to the immutable OCI assignment. CPU and memory limits come from the applied sandbox resources. Each sample is revalidated against the same runtime identity after collection; retiring instances and expired consumer leases are excluded. A pause, resume, or new runtime generation starts a separate counter series.
...
- node-local ctld collectors for sandbox-wide stock-runsc runtime samples,
