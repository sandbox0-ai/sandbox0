...
spec:
    ...
    mainContainer:
        image: registry.example.com/t-<team-key>/my-ds-env:v2.0
        resources:
            memory: 8Gi
...
| `resources.ephemeralStorage` | string | `8Gi` | Writable-layer and container-log storage for the sandbox runtime. Checkpointed pause/resume saves and restores the writable root filesystem, but this quota still bounds writes to `/tmp` or image-local paths. Use rootfs snapshots and claim-time `snapshot_id` to version initialized sandbox files. |
...
Warm pool configuration. See [Warm Pool](/docs/sandbox/template/pool) for a detailed guide.

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `minIdle` | integer | — | Minimum idle pods to pre-warm. Required (`>= 0`). |
| `maxIdle` | integer | — | Compatibility ceiling for idle pods and status health. Required (`>= minIdle`). |
...
See [Network](/docs/sandbox/network) and [Protocol Controls](/docs/sandbox/protocol-controls) for traffic behavior, and [Credential](/docs/sandbox/credential) for bindings and egress auth.
...
`pod.emptyDirMounts` is backed by Kubernetes `emptyDir` on the node's default storage medium. Its contents are tied to the sandbox pod lifecycle and are not captured by rootfs snapshots or restore operations. Use it for temporary runtime state and caches, not durable workspace data.
...
<CardGroup>
  <Card title="Snapshot And Restore" href="/docs/sandbox/snapshot-restore" cta="Continue">
    Publish, restore, and fork durable sandbox rootfs state.
  </Card>
</CardGroup>
