...
func Registry() []Entry {
	return []Entry{
		...
		prefix("spec.observability", "observability", []string{"global-gateway", "regional-gateway", "ssh-gateway", "scheduler", "cluster-gateway", "manager", "storage-proxy", "netd", "ctld"}, []string{"platform service environment", "InfraPlan.Components.EnableObservability"}, UpdateSemanticsDeclarative, "Standard OpenTelemetry collection env injection and optional observability backend integration. User-facing query APIs and adapters are out of scope."),
		prefix("spec.observability.backend", "observability", []string{"observability", "global-gateway", "regional-gateway", "ssh-gateway", "scheduler", "cluster-gateway", "manager", "storage-proxy", "netd", "ctld", "status"}, []string{"InfraPlan.Components.EnableObservability"}, UpdateSemanticsDeclarative, "Controls disabled, builtin, or external observability backend integration."),
		prefix("spec.observability.backend.builtin", "observability", []string{"observability", "status"}, nil, UpdateSemanticsMixed, "Builtin ClickHouse and managed Collector resources are reconciled by the observability service."),
		...
		prefix("spec.observability.traces", "observability", []string{"global-gateway", "regional-gateway", "ssh-gateway", "scheduler", "cluster-gateway", "manager", "storage-proxy", "netd", "ctld"}, []string{"platform service environment"}, UpdateSemanticsDeclarative, "Legacy direct trace exporter env injection. Explicit fields override backend-derived trace defaults."),
