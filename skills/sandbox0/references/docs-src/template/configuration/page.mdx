...
Only Sandbox0-ready idle pods count toward pool capacity. This matters when your template uses sidecars to host a warm process.
...
| `readinessProbe` | Optional probe syntax for sidecars. Sandbox0 executes it and uses the result for the sandbox-managed readiness gate. |
| `startupProbe` | Optional startup probe for slow-starting sidecars. |

<Callout variant="info">
Sandbox0 owns sandbox pod readiness. A pod that is `Running` but not `Ready` does not count as available warm-pool capacity and will not be hot-claimed.
</Callout>

<Callout variant="warning">
`livenessProbe` is not part of the public template surface. Whole-sandbox soft pause freezes the sandbox, so kubelet liveness semantics would conflict with pause/resume.
</Callout>
...
Regular team-owned templates can still use public sidecar fields such as `command`, `args`, `env`, `resources`, `mounts`, and `startupProbe`.
