...
func assertInvalidCredentialRuleIsDropped(t *testing.T, rule v1alpha1.EgressCredentialRule, binding v1alpha1.CredentialBinding) {
	t.Helper()
	...
	result := svc.BuildNetworkPolicyState(&BuildNetworkPolicyRequest{
		...
		RequestSpec: &v1alpha1.SandboxNetworkPolicy{
			...
			Egress: &v1alpha1.NetworkEgressPolicy{
				CredentialRules: []v1alpha1.EgressCredentialRule{rule},
			},
		},
		RequestBindings: []v1alpha1.CredentialBinding{binding},
	})
	...
	if len(result.PolicySpec.Egress.CredentialRules) != 0 {
		t.Fatalf("rules = %#v, want invalid rule dropped", result.PolicySpec.Egress.CredentialRules)
	}
	if len(result.CredentialBindings) != 0 {
		t.Fatalf("bindings = %#v, want invalid binding dropped", result.CredentialBindings)
	}
}
...
func TestBuildNetworkPolicyStateDropsPlaceholderSubstitutionForNonHTTPRule(t *testing.T) {
	assertInvalidCredentialRuleIsDropped(t, v1alpha1.EgressCredentialRule{
		Name:          "api-placeholder",
		CredentialRef: "api-token",
