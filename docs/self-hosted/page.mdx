...
- **Data plane** (runtime): `cluster-gateway`, `manager` with the storage API runtime, a node-local `ctld` HA pair for rootfs checkpoint writes, volume portals, and network policy, plus the Sandbox0 containerd snapshotter for metadata-head reads
...
In single-cluster fullmode, the storage API runtime runs in the `manager` Deployment. Each sandbox node has `ctld-a` and `ctld-b` DaemonSet Pods; the elected primary continuously persists writable-rootfs changes, seals metadata heads, and materializes a selected head directly into node-local containerd while owning volume portals and network policy. The synchronized standby assumes those responsibilities after promotion. A separate external containerd snapshotter lazily serves immutable metadata heads from S3/OSS object ranges. PostgreSQL and S3/OSS are rootfs lifecycle dependencies. The optional image registry is used for template image publication and cold base pulls, not warm marker-head pause/resume.
...
- `spec.registry` when template image publication or an internal base-image registry is required; rootfs marker heads do not use it
