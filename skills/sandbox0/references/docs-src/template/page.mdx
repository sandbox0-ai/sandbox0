...
Warm processes start during template pod creation, before the pod is claimable from the warm pool. They are intended for long-running helpers such as agents, language servers, and daemon-style workers; if a configured warm process exits, procd readiness fails and the pod is not hot-claimed.
