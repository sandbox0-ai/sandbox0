...
The signed audit ledger originally started at schema v2 in the dedicated `sandbox_audit_events` table. Older `sandbox_events` rows do not have trustworthy actor or signature data and are retained as legacy observability history rather than being retroactively signed. Schema v3 adds a signed native execution scope; unscoped events continue to use v2. Startup migration adds the scope columns without rewriting historical v2 rows, which remain verifiable and have no scope. Public list and watch requests default to a maximum schema of v2 until a client requests v3 or supplies an execution-scope filter. Existing CRs whose persisted table value is the old default are routed to the canonical table. At startup, audit-enabled cluster-gateway validates the actual ClickHouse engine, partition key, sorting key, and required columns, and refuses to serve audit operations against an incompatible custom table.

For a rolling upgrade, finish upgrading every cluster-gateway replica in a region before enabling the new netd attribution path. The upgraded gateway remains a v2-compatible reader by default and applies the schema migration; upgrading netd afterward is what first introduces scoped v3 facts. Release or upgrade v3-aware SDK clients only after that regional server rollout is complete.
...
Gateway request rate limiting defaults to process-local memory. Configure `spec.redis` when gateway replicas need a shared Redis backend. The ctld network runtime uses the same Redis backend when cluster-scoped team bandwidth limits are configured.
...
- `network.config.teamEgressBandwidthBytesPerSecond`, `teamIngressBandwidthBytesPerSecond`, and `teamBandwidthBurstBytes` apply cluster-scoped per-team aggregate throttling when set above zero and `spec.redis` is configured
