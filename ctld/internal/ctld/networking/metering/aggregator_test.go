...
func TestAggregatorFlushesIngressAndEgressWindows(t *testing.T) {
	...
	if recorder.tx.watermarkProducer != "ctld.network.byte_windows/node-1" {
		t.Fatalf("producer = %q, want %q", recorder.tx.watermarkProducer, "ctld.network.byte_windows/node-1")
	}
