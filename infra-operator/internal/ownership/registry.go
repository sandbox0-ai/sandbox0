...
func Registry() []Entry {
	return []Entry{
		...
		prefix("spec.observability", "observability", []string{"global-gateway", "regional-gateway", "ssh-gateway", "scheduler", "cluster-gateway", "manager", "storage-proxy", "netd", "ctld"}, []string{"platform service environment"}, UpdateSemanticsDeclarative, "Standard OpenTelemetry collection env injection for sandbox0 platform services; storage and query backends are out of scope."),
