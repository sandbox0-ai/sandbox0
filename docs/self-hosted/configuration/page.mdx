...
Set `spec.sandboxObservability.enabled` to `false` or omit the field to keep the historical API disabled. Set `spec.metering.enabled` to `false` or omit `spec.metering` to keep metering producers, quota usage reads, and metering export readers disabled. Legacy `spec.sandboxObservability.type` fields are still accepted for compatibility, but new installs should use `spec.clickHouse` for ClickHouse provisioning. Retention deletes ClickHouse observability rows only and never deletes metering ledger rows.

When the backend is enabled, infra-operator injects bounded internal ingest endpoints into the producers automatically: ctld sends sandbox-wide CRI runtime samples, manager sends historical logs, and netd sends network audit events. Producer queues never block the sandbox data path; a full queue or exhausted retry budget drops projection data instead. Netd exports `sandbox0_netd_audit_ingest_events_total` and `sandbox0_netd_audit_ingest_batches_total` to show enqueue, send, skip, drop, and failed batch results.
...
For high-density nodes, `ctld` can serve all volume portals through a fixed set of node-wide FUSE connections instead of creating one connection per Sandbox:
...
This mode does not deploy another service. `ctld` owns the shard connections, keeps their lifecycle journal on its existing host data path, and bind-mounts each portal subtree into the CSI target. It works with both gVisor and host runtimes such as runc because the runtime receives a normal CSI mount.

`requireFUSERecovery: true` is mandatory whenever `nodeFSShardCount` is positive. It requires a kernel with Sandbox0's FUSE recovery patch queue and refuses to start nodefs if the kernel does not negotiate recovery, request resend, recovered-request status, cache-domain invalidation, and empty-domain registration. The maintained [`sandbox0-ai/linux` branch](https://github.com/sandbox0-ai/linux/tree/sandbox0/linux-6.18.y-fuse-recovery) is a linear patch queue on the official Linux stable `linux-6.18.y` branch; it is not tied to a distribution or cloud provider. Build or package that branch once for the node architecture, then install and boot it on a drained canary node before enabling the setting. Drain a node before enabling nodefs, disabling it, or changing `nodeFSShardCount`; the shard count is immutable while that node has committed portals.

Monitor `sandbox0_ctld_nodefs_slot_remaining`, `sandbox0_ctld_nodefs_portals_active`, and `sandbox0_ctld_nodefs_binding_generation_remaining`. On startup, ctld automatically rotates the connection generation when the shard set is empty and 75% of its portal slots have been consumed. Drain and restart a running node before `nodefs_slot_remaining` reaches zero; draining also provides the rollback boundary for a kernel or shard-count change.
