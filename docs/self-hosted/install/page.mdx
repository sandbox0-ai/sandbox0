...
- `infra-operator` runs two independent ctld DaemonSets per sandbox node. One process owns the node endpoint and FUSE request channels; the synchronized standby preserves existing mounts across a single ctld process or Pod failure. A filesystem call already being processed by the failed process can return an error and should be retried. Node reboot, kernel failure, and simultaneous loss of both ctld Pods remain outside this protection boundary.
