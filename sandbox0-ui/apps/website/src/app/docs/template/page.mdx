...
A Template is the blueprint for creating Sandboxes. It defines the container image, resource limits, warm pool size, default network policy, and lifecycle behavior. Every Sandbox is created from a Template.
...
Builtin templates use curated public images and are ready to use immediately. Custom templates let you bring your own image, fine-tune resources, and set lifecycle defaults tailored to your workload.
...
tpl = client.templates.create(TemplateCreateRequest(
...
fmt.Printf("Template: %s (scope: %s)\\n", tpl.TemplateID, tpl.Scope)`
    },
    {
      label: "Python",
      language: "python",
      code: `tpl = client.templates.get("my-python-env")
print(f"Template: {tpl.template_id} (scope: {tpl.scope})")`
    },
    {
      label: "TypeScript",
      language: "typescript",
      code: `const tpl = await client.templates.get("my-python-env");
...
for _, tpl := range templates {
    ...
    },
    {
      label: "Python",
      language: "python",
      code: `templates = client.templates.list()
...
fmt.Printf("Template updated: %s\\n", tpl.TemplateID)`
    },
    {
      label: "Python",
      language: "python",
      code: `tpl = client.templates.update(
    "my-python-env",
...
print(f"Template updated: {tpl.template_id}")`
    },
    {
      label: "TypeScript",
      language: "typescript",
      code: `const tpl = await client.templates.update("my-python-env", {
    spec: {
        mainContainer: {
            image: "python:3.12-slim",
            resources: { cpu: "2", memory: "4Gi" },
...
fmt.Println("Template deleted")`
    },
    {
      label: "Python",
      language: "python",
      code: `client.templates.delete("my-python-env")
