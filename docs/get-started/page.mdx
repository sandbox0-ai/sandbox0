...
`manager` owns lifecycle and rootfs metadata. On each sandbox node, the active process in the `ctld` HA pair owns rootfs persistence and the network policy runtime; the standby takes over those responsibilities after promotion.
