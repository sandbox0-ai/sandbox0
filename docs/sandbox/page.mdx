...
| `max_schema_version` | integer | Highest event schema the client can decode, minimum `2`. Defaults to `2`; values newer than the server are capped to its current v3 |
| `execution_scope_namespace` | string | Exact logical execution namespace, such as `codex`; infers v3 when `max_schema_version` is omitted |
| `execution_scope_kind` | string | Exact native scope kind, such as `native_session`; infers v3 when `max_schema_version` is omitted |
| `execution_scope_id` | string | Exact native execution identifier; infers v3 when `max_schema_version` is omitted |
| `execution_scope_attribution` | string | Attribution signal, currently `process_environment`; infers v3 when `max_schema_version` is omitted |
...
Every audit fact includes a stable `event_id`, schema version, trusted actor, action, resource, producer, operation correlation, outcome, and bounded request metadata. Unscoped facts use schema v2. Schema v3 carries a signed `execution_scope` that partitions activity from multiple native agent sessions sharing one sandbox. The scope does not replace the actor: the sandbox workload remains the actor, while namespace, kind, ID, and attribution describe the logical execution that produced the activity. Historical and newly produced unscoped v2 facts remain queryable and verifiable.

List and watch requests default `max_schema_version` to `2`, so older generated clients never receive a schema enum they cannot decode. A client that supports execution scopes can opt in with `max_schema_version=3`; the response may then contain both v2 and v3 facts. Omitting the parameter while supplying any `execution_scope_*` filter safely infers v3, while explicitly combining such a filter with v2 is rejected. Pagination and watch cursors bind the effective schema ceiling; cursors created before this negotiation field existed are treated as v2, and changing the ceiling while resuming is rejected.

A list response always returns `data.effective_query`, including when `events` is empty. It contains the effective schema ceiling plus the normalized `execution_scope_*` filters the server applied. An audit-event watch negotiated at v3 sends a `ready` line carrying the same `effective_query` before every event, watermark, or heartbeat line. The default v2 watch retains its legacy frame sequence and does not emit `ready`, so pre-v3 SDKs never encounter the new frame enum. Clients that require scoped audit isolation must negotiate v3 and can fail closed when the first frame is not the expected acknowledgement, instead of guessing support from event content.

Pagination cursors and watch watermarks are response transport metadata and are not repeated inside each signed event. `integrity` contains the canonical SHA-256 payload digest, Ed25519 signature, signing key ID, and query-time `signature_status`. List and watch responses recompute each returned row's digest and verify its signature. An exact `event_id` lookup fetches enough rows to set `event_id_conflict=true` when the same stable ID appears with multiple payload hashes; this marker does not overwrite `signature_status`, so callers can distinguish payload collisions from signature failures.
...
Network allow decisions are fsynced to a node-local spool before the ctld network runtime opens the upstream connection. In `durable_async`, canonical delivery follows asynchronously; in `canonical_sync`, ClickHouse must acknowledge the allow attempt first, including for server-first SSH probes. A ClickHouse outage therefore delays audit-query visibility in `durable_async` but denies new flows in `canonical_sync`; an unwritable local buffer also fails closed unless synchronous canonical fallback succeeds. The result is appended when the flow closes. Consequently a long-lived keep-alive connection has one allow attempt and one eventual flow result; parsed HTTP or MCP operations may appear inside its bounded protocol details rather than as separate connections. Durable records created before schema v3 omitted an explicit schema field; after an upgrade they replay as v2 so their canonical payload remains stable instead of being rewritten as a new v3 fact. Raw proxy and credential-resolution error strings remain local diagnostics and are not copied into the canonical ledger because they may contain upstream or credential details.

When a supervised session declares `execution_scope`, the network runtime resolves each new flow to a descendant process and reads only the configured marker from that process ancestry. The resulting attribution is frozen for the flow's attempt and result. UDP tuples are revalidated at most once per second rather than putting every datagram on the process-resolution path. A transient lookup failure may retain the last confirmed UDP attribution for up to five seconds; after that the runtime rotates the flow to an unattributed one. Missing, ambiguous, timed-out, or conflicting resolution otherwise leaves the event unattributed and never blocks network traffic. `process_environment` is an observed marker, not an unforgeable identity. Connections opened directly by a shared in-process agent server cannot be separated by native session unless that harness propagates a process or trusted runtime boundary.
