...
func Registry() []Entry {
	return []Entry{
		...
		prefix("spec.internalAuth", "internal-auth", []string{"internal-auth", "regional-gateway", "function-gateway", "scheduler", "cluster-gateway", "manager", "storage-proxy", "netd", "ssh-gateway"}, nil, UpdateSemanticsDeclarative, "Controls generated or imported internal JWT keys consumed by control-plane and data-plane services."),
		...
		prefix("spec.services.functionGateway", "function-gateway", []string{"function-gateway", "status"}, nil, UpdateSemanticsDeclarative, "Direct runtime configuration for function-gateway."),
		...
		exact("spec.services.clusterGateway.service.type", "plan", []string{"cluster-gateway", "regional-gateway", "function-gateway"}, []string{"InfraPlan.Services.ClusterGateway.Port", "validation"}, UpdateSemanticsDeclarative, "Service type determines how the cluster-gateway address is projected downstream."),
		exact("spec.services.clusterGateway.service.port", "plan", []string{"cluster-gateway", "regional-gateway", "function-gateway"}, []string{"InfraPlan.Services.ClusterGateway.Port", "InfraPlan.RegionalGateway.DefaultClusterGatewayURL", "InfraPlan.FunctionGateway.DefaultClusterGatewayURL", "validation"}, UpdateSemanticsDeclarative, "Cluster-gateway service port is projected into regional-gateway and function-gateway upstream config."),
		exact("spec.services.functionGateway.service.port", "plan", []string{"function-gateway", "regional-gateway"}, []string{"InfraPlan.Services.FunctionGateway.Port", "InfraPlan.RegionalGateway.FunctionGatewayURL"}, UpdateSemanticsDeclarative, "Function-gateway service port is projected into generated regional-gateway config for compatibility."),
