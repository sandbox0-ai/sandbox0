...
Keep a regular node pool for core system services. In the GKE `gVisor` sample, `cluster-gateway`, `manager`, PostgreSQL, and the builtin registry run on the regular pool. The ctld HA pair and sandbox workloads run on the `gVisor` node pool, while ctld itself uses the node's host-compatible runtime.
...
3. `fullmode-ctld-a` and `fullmode-ctld-b` run on `gvisor-lite`; the elected primary owns rootfs and network runtime responsibilities
4. sandbox pods in `tpl-default` run on `gvisor-lite`

`fullmode-manager` serves lifecycle and rootfs metadata APIs. On every sandbox node, the elected ctld primary owns rootfs persistence and network enforcement; the standby takes over after promotion.
