...
// enqueueSandboxDeletionWebhook persists the exact signed request that will be
...
func enqueueSandboxDeletionWebhook(ctx context.Context, tx pgx.Tx, sandboxID, teamID, targetURL, secret string, deletedAt time.Time) error {
...
func (o *SandboxDeletionWebhookOutbox) claim(ctx context.Context, workerID string, limit int, claimTTL time.Duration) ([]sandboxDeletionWebhookDelivery, error) {
	...
	rows, err := o.pool.Query(ctx, `
		...
		FROM claimed
	`, workerID, limit, durationSeconds(claimTTL))
	if err != nil {
...
func (o *SandboxDeletionWebhookOutbox) recordRetry(ctx context.Context, eventID, workerID string, status int, lastError string, delay time.Duration) error {
	return o.updateClaim(ctx, eventID, workerID, `
		...
		SET attempts = attempts + 1,
			...
			updated_at = NOW()
		WHERE event_id = $1 AND claimed_by = $2
	`, nullableWebhookStatus(status), durationSeconds(delay), truncateSandboxDeletionWebhookError(lastError))
}
