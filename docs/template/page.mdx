...
Builtin templates use curated public images and are ready to use immediately. Custom templates let you bring your own image, fine-tune resources, configure environment variables, and define default network policy.
...
Creating a template from a sandbox captures its current writable root filesystem as an immutable regional block-COW generation. The template keeps the original digest-pinned base image as its runtime identity and restores the captured generation for each new sandbox. This is useful when initialization is interactive or agent-driven and you want the result to become a normal team template with its own warm-pool settings.
...
- claiming with `snapshot_id` creates a new running sandbox; the requested template still supplies the image, default resources, services, and network defaults
...
Create a custom template from the current root filesystem of an existing sandbox. Sandbox0 captures and publishes an immutable block-COW generation asynchronously, so the endpoint returns `202 Accepted` with a template whose `status.creation.state` is `creating`.
...
The new template inherits safe settings from the source sandbox's template. Sandbox0 does not copy sandbox identity, active processes, memory contents, sessions, claim-time environment or network overrides, services, or privileged pod and security fields. If `pool` is omitted, it defaults to `minIdle: 0` and `maxIdle: 0`.

The source sandbox may be running or paused. Because capture is asynchronous, request acceptance is not the rootfs capture point. Keep the source sandbox available while the template is in the `capturing` stage. Sandbox0 briefly barriers the exact writer of a running source and publishes an immutable checkpoint; concurrent writes continue after that short barrier. `status.creation.capturedAt` identifies the committed capture point. The source can then continue changing or be deleted without changing the template.

<Callout variant="info">
You do not provide an image repository or registry credentials. Sandbox0 retains the immutable capture in the team's region-scoped RootFS store. Every cluster serving that region shares the PostgreSQL and object-store authority needed to claim it.
</Callout>

<Callout variant="warning">
The source and target warm-slot profiles must use the same attested platform and RootFS format generation. Capture fails closed if the base artifact, source image digest, platform, writer grant, allocation, or node incarnation changes before publication.
</Callout>
...
| `creating` | `publishing` | The immutable RootFS capture is being bound to the template; image-publisher backends assemble and push OCI content during this stage |
| `creating` | `reconciling` | The completed RootFS representation is being made claimable by data-plane clusters |
| `ready` | `reconciling` | The claim API accepts the template; runtime-native captures retain this last stage as provenance |
...
With a zero-sized pool, `ready` does not mean the image or captured RootFS is already resident on a sandbox node. The first claim still performs the normal RootFS attach and sandbox startup path.

Deleting or cancelling a template first records a durable cleanup tombstone, then releases Sandbox0's internal RootFS snapshot pin. Shared generation objects remain reachable while any sandbox created from the template still uses them and are reclaimed asynchronously after the last derived filesystem is gone.
