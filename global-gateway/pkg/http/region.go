...
var errInitUserHomeRegionLookupUnavailable = errors.New("home region lookup unavailable")
var errInitUserHomeRegionRequired = errors.New("home_region_id is required")
var errInitUserHomeRegionInvalidFormat = errors.New("home region id must use provider-region format")
var errInitUserHomeRegionNotRoutable = errors.New("home region is not routable")

// ValidateInitUserHomeRegion verifies the configured init user's home region for global-gateway bootstrap.
func ValidateInitUserHomeRegion(ctx context.Context, regionLookup regionDirectory, homeRegionID string) error {
	if strings.TrimSpace(homeRegionID) == "" {
		return errInitUserHomeRegionRequired
	}
	if !tenantdir.IsNormalizedRegionID(homeRegionID) {
		return errInitUserHomeRegionInvalidFormat
	}
	if regionLookup == nil {
		return errInitUserHomeRegionLookupUnavailable
	}
	...
	if region == nil || !region.Enabled || strings.TrimSpace(region.RegionalGatewayURL) == "" {
		return errInitUserHomeRegionNotRoutable
	}
	return nil
}
...
func (h *RegionHandler) CreateRegion(c *gin.Context) {
	...
	if !tenantdir.IsNormalizedRegionID(region.ID) {
...
func (h *RegionHandler) UpdateRegion(c *gin.Context) {
	...
	if req.RegionalGatewayURL != "" {
		region.RegionalGatewayURL = strings.TrimSpace(req.RegionalGatewayURL)
	}
