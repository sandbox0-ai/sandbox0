...
You can still pause a sandbox manually. If the sandbox has `auto_resume: true` and the service route has `resume: true`, a later public request can create a new runtime pod and restart the `cmd` service. Running processes, sockets, memory, and in-flight connections are not preserved across pause/resume; durable state should live in the sandbox rootfs or an external system.
