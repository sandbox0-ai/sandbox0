...
- `optional` lets a claim omit this mount point. Omitted optional mounts stay as unbound portal directories for the lifetime of that sandbox.
- `/` is not allowed.
...
`mount_point` must match a path declared in `spec.volumeMounts`. Template-declared mounts are required by default. Sandbox0 rejects claims that omit required mounts, while mounts declared with `optional: true` may be left unbound.

Only mounts present in the claim request are treated as bound Sandbox Volume paths for that Sandbox. Bound Sandbox Volume paths are excluded from rootfs pause, snapshot, restore, and fork checkpoints because their contents are owned by the Sandbox Volume. Unbound optional portal directories are not treated as Sandbox Volume contents.
