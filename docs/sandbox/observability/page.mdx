...
On Nomad nodes, ctld samples stock `runsc` and the sandbox's resource cgroup only after matching the current writer and carrier journals to the immutable OCI assignment. CPU and memory limits come from the applied sandbox resources. Memory usage comes from `runsc`; working set is derived from cgroup-v2 `memory.current` minus `inactive_file`, clamped to zero across a sampling race. Each sample is revalidated against the same runtime identity after collection; retiring instances and expired consumer leases are excluded. A pause, resume, or new runtime generation starts a separate counter series.
...
| `sandbox.rootfs.writable.usage` | gauge | bytes | Writable-layer usage when supported by the runtime; omitted when stock `runsc` does not report it |
| `sandbox.rootfs.writable.inodes` | gauge | count | Writable-layer inodes when supported by the runtime; omitted when stock `runsc` does not report them |
...
| `cursor` | string | Opaque cursor from a previous response; in watch mode, use a `watermark` cursor |
| `watch` | boolean | Stream matching records as NDJSON until the client disconnects |
| `source` | string | Trusted producer, such as `cluster_gateway` or `ctld`; network audit events are produced by ctld's network runtime |
...
Gateway replay and foreground requests share a bounded pool of canonical writers. An in-flight replay of the same event shares its acknowledgement with the request; replay of unrelated events does not hold a global admission lock. Mutations still require the applicable ClickHouse acknowledgement before proceeding.
...
- node-local ctld collectors for sandbox-wide runsc and resource-cgroup runtime samples,
  collected every 15 seconds with jitter
...
- Cluster-gateway pending events require reattachable durable storage.
  Node-local network-flow buffers survive ctld process restarts on the same
  disk, not permanent node loss.
