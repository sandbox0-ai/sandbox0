...
func (m *iptablesManager) Cleanup(ctx context.Context) error {
	...
	if err := m.cleanupChains(ctx, chainName, natChainName); err != nil {
		return err
	}
	for _, name := range []string{ipsetName, nextIPSetName} {
		if err := destroyIPSet(ctx, name); err != nil {
			return err
		}
	}
...
func (m *iptablesManager) cleanupChains(ctx context.Context, mangleChain, natChain string) error {
	_ = ctx
	if err := m.ipt.DeleteIfExists(mangleTable, "PREROUTING", "-j", mangleChain); err != nil {
		return err
	}
	if exists, err := m.ipt.ChainExists(mangleTable, mangleChain); err != nil {
		return err
	} else if exists {
		if err := m.ipt.ClearChain(mangleTable, mangleChain); err != nil {
			return err
		}
		if err := m.ipt.DeleteChain(mangleTable, mangleChain); err != nil {
			return err
		}
	}
	if err := m.ipt.DeleteIfExists(natTable, "PREROUTING", "-j", natChain); err != nil {
		return err
	}
	if exists, err := m.ipt.ChainExists(natTable, natChain); err != nil {
		return err
	} else if exists {
		if err := m.ipt.ClearChain(natTable, natChain); err != nil {
			return err
		}
		if err := m.ipt.DeleteChain(natTable, natChain); err != nil {
			return err
		}
	}
	return nil
...
func destroyIPSet(ctx context.Context, name string) error {
	...
	return fmt.Errorf("destroy ipset %s: %s: %w", name, strings.TrimSpace(string(out)), err)
}
