...
On nodes using the containerd overlayfs snapshotter, `ctld` checkpoints only the active snapshot upperdir and stores it as a standard OCI layer diff. This is the supported bounded checkpoint path. A different snapshotter, or an overlayfs upperdir that `ctld` cannot resolve and read, fails the checkpoint; Sandbox0 does not fall back to containerd's potentially unbounded built-in diff path.
