...
Sandbox0 wires warm process state into the pod's Kubernetes probes. If a warm process exits, `/readyz` fails and the pod leaves the warm pool. `/healthz` also fails, so Kubernetes restarts `procd`; during restart, `procd` starts the configured warm processes again before the pod becomes Ready.
