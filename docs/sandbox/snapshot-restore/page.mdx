...
Snapshots, snapshot claims, restore, and fork use the same block-COW
persistence boundary as pause and resume. Ordinary writable RootFS paths,
including `/tmp`, `/var/tmp`, and workspace directories, are captured. Virtual
runtime mounts such as `/proc`, `/dev`, and `/sys` are recreated for each
runtime generation and are not snapshot content.
