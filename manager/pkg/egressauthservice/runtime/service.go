...
type Service struct {
	...
	logger       *zap.Logger
	bindingStore egressauthstore.BindingStore
	staticAuth   map[string]StaticAuthConfig
	providers    map[string]provider
}

func NewService(cfg Config, bindingStore egressauthstore.BindingStore, logger *zap.Logger) *Service {
	if logger == nil {
		logger = zap.NewNop()
	}
...
func (s *Service) resolveBinding(ctx context.Context, req *egressauth.ResolveRequest, binding *egressauthstore.CredentialBinding) (*egressauth.ResolveResponse, error) {
	...
	response := egressauth.CloneResolveResponse(result.Response)
...
func (s *Service) resolveStatic(req *egressauth.ResolveRequest) (*egressauth.ResolveResponse, error) {
	...
	response := newHTTPHeadersResolveResponse(entry.AuthRef, entry.Headers, &expiresAt)
	return response, nil
}

func (s *Service) lookupBinding(ctx context.Context, req *egressauth.ResolveRequest) *egressauthstore.CredentialBinding {
