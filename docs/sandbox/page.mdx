...
Get live, best-effort resource usage for a sandbox. This API calls the sandbox runtime and is intended for live debugging, not historical analysis.

<Endpoint method="GET">
/api/v1/sandboxes/{'{id}'}/stats
</Endpoint>

The response includes aggregate container and context resource usage. For per-context stats, use `/api/v1/sandboxes/{'{id}'}/contexts/{'{ctx_id}'}/stats`.

---

## Historical Observability and Audit

Historical sandbox observability is separate from live logs and live stats. The query API reads an asynchronous per-sandbox projection that can be backed by ClickHouse or another query store. When no historical backend is configured, these endpoints return `503 unavailable`. When the backend is configured but cannot be queried, the same endpoints return `503 unavailable` with a backend-unavailable error.
