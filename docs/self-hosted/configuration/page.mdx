...
For an advanced external ClickHouse deployment, `spec.sandboxObservability.audit.storagePolicy` can assign only `sandbox_audit_events` to a pre-provisioned ClickHouse storage policy. Cluster-gateway applies the setting during both table creation and schema reconciliation, so the named policy must exist before schema migration starts. Leave it unset to inherit ClickHouse's default MergeTree policy. Logs, runtime samples, and metering tables are never moved by this audit-specific setting.
